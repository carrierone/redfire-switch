...
use redfire_sip_stack::{
    SipParser, SipMessage, SipMethod, SipStatusCode,
    SipTransport, TransportConfig, SipTransportManager,
    SipCoreEngine, SipCoreConfig, SipCallContext,
    SipStateManager, SipStateConfig,
    SipAuthenticator, AuthResult,
};

// Import codec engine components
use redfire_codec_engine::{
    CodecService, CodecConfig, AudioCodec,
    ResamplingService, ResamplerConfig,
};
...
use crate::rtp_proxy_impl::{RtpProxyService, RtpProxyConfig, MediaSession};
...
impl SipCodecIntegration {
    ...
    pub async fn process_sip_message(
        ...
        if let Some(ref method) = method {
            let auth_result = self.sip_auth.lock().await.authenticate_request(
                &sip_message.message, 
                from_addr.ip()
            ).await?;
            match auth_result {
                AuthResult::Authorized { trunk_id, customer_id, .. } => {
                    debug!("Request authenticated successfully: {} / {}", trunk_id, customer_id);
                }
                AuthResult::Challenge { realm, nonce, .. } => {
                    ...
                    return self.send_auth_challenge(challenge, from_addr, transport).await;
                }
        ...
        match method {
            Some(SipMethod::Invite) => {
                self.handle_invite(sip_message, from_addr, transport).await?;
            }
            Some(SipMethod::Bye) => {
            ...
            Some(SipMethod::Cancel) => {
                self.handle_cancel(sip_message, from_addr, transport).await?;
            }
            Some(SipMethod::Options) => {
                self.handle_options(sip_message, from_addr, transport).await?;
            }
            _ => {
    ...
    async fn handle_invite(
        ...
        if requires_transcoding {
            info!("Call {} requires transcoding: {:?} -> {:?}", 
        ...
        let media_session = if self.has_sdp(&message) {
            ...
            let (ingress_local, egress_local) = self.rtp_proxy.start_session(
                call_id.clone(),
                call_id.clone(),
                ingress_remote,
                egress_remote,
            ...
            info!("RTP proxy session established: {} <-> {}", 

...
use crate::q931_messages::{IsdnVariant, IsdnSideType, Q931MessageType};
...
impl PriTimerType {
    ...
    pub fn default_duration(&self, variant: IsdnVariant, side: IsdnSideType) -> Duration {
        match (self, variant, side) {
            ...
            (PriTimerType::T310, IsdnVariant::NI2, IsdnSideType::Network) => Duration::from_secs(40),
...
impl PriTimerManager {
    ...
    pub async fn start_timer(
        ...
        let timer_duration = duration.unwrap_or_else(|| {
            timer_type.default_duration(self.variant, self.side_type)
        });
        ...
        self.active_timers.write().await.insert(timer_id.clone(), timer.clone());
        
        // Start timer task
        let timer_handle = self.spawn_timer_task(timer_id.clone(), timer).await;
        self.timer_handles.write().await.insert(timer_id.clone(), timer_handle);
        
        info!("Started PRI timer {}: {} on CRV {} ({})", 
              timer_id, timer_type.description(), call_reference, 
    ...
    pub async fn cancel_call_timers(&self, call_reference: u16) -> usize {
        ...
        if cancelled_count > 0 {
            info!("Cancelled {} PRI timers for CRV {}", cancelled_count, call_reference);
        }
    ...
    pub async fn cancel_circuit_timers(&self, circuit_id: u16) -> usize {
        ...
        if cancelled_count > 0 {
            info!("Cancelled {} PRI timers for circuit {}", cancelled_count, circuit_id);
        }
    ...
    fn generate_timer_id(&self, timer_type: PriTimerType, call_reference: u16, circuit_id: u16) -> String {
        format!("{:?}-CRV{}-C{}", timer_type, call_reference, circuit_id)
    }
    
    /// Spawn timer task for specific timer
    async fn spawn_timer_task(&self, timer_id: String, timer: ActivePriTimer) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            if active_timers.write().await.remove(&timer_id).is_some() {
                ...
                warn!("PRI Timer {} expired: {} on CRV {} - Action: {:?}",
                      timer_id, timer.timer_type.description(), 
                      timer.call_reference, timer.action);
            }
        })
    }
...
mod tests {
    ...
    async fn test_timer_creation_and_expiration() -> Result<()> {
        ...
        let timer_id = manager.start_timer(
            PriTimerType::T303,
            123, // CRV
            1,   // Circuit ID

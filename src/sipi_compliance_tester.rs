...
use redfire_sip_stack::sipt_sipi::{
    IsupMessage, IsupMessageType, IsupParameterType, IsupVariant, SipTSipIConfig, SipTSipIService,
};
...
impl SipIComplianceTester {
    pub async fn new(
        ...
        info!(
            "SIP-I service initialized for compliance testing - SIP-I: {}, SIP-T: {}",
    ...
    async fn run_sipi_test(&self, test: &SipIComplianceTest) -> SipITestResult {
        ...
        match test.test_type {
            SipITestType::SipToIsup => self.test_sip_to_isup_conversion(test, &mut details).await,
            SipITestType::IsupToSip => self.test_isup_to_sip_conversion(test, &mut details).await,
            SipITestType::BiDirectional => self.test_bidirectional_flow(test, &mut details).await,
            SipITestType::ParameterMapping => self.test_parameter_mapping(test, &mut details).await,
            SipITestType::ErrorHandling => self.test_error_handling(test, &mut details).await,
        }
    }

    async fn test_sip_to_isup_conversion(
        &self,
        test: &SipIComplianceTest,
        details: &mut SipITestDetails,
    ) -> SipITestResult {
        // Create test SIP INVITE with phone numbers
        let sip_invite = self
            .create_test_sip_invite("+15551234567", "+15559876543", &test.test_id)
            .await;
        details.sip_message_sent = Some(sip_invite.clone());

        // Send SIP INVITE and analyze response
        match self
            .send_and_analyze_sipi_response(&sip_invite, &["100", "180", "183"])
            .await
        {
            Ok((response, analysis)) => {
                ...
                let status = if let Some(expected_type) = &test.expected_isup_message {
                    if analysis.isup_message_type == Some(*expected_type) {
                        details.parameter_mapping_correct = self.validate_required_parameters(
                            &analysis.isup_parameters_found,
                ...
                SipITestResult {
                    ...
                    status,
                    message: format!(
                        "SIP to ISUP test: ISUP detected={}, Type={:?}",
    ...
    async fn test_isup_to_sip_conversion(
        &self,
        test: &SipIComplianceTest,
        details: &mut SipITestDetails,
    ) -> SipITestResult {
        // Create test SIP message with ISUP content
        let isup_message = self
            .create_test_isup_message(test.expected_isup_message.unwrap_or(IsupMessageType::ACM))
            .await;
        let sip_with_isup = self
            .create_sip_with_isup(&isup_message, &test.test_id)
            .await;
        ...
        // Send and analyze
        match self
            .send_and_analyze_sipi_response(&sip_with_isup, &["100", "180", "183", "200"])
            .await
        {
            Ok((response, analysis)) => {
                details.sip_response_received = Some(response.clone());
    ...
    async fn test_parameter_mapping(
        &self,
        test: &SipIComplianceTest,
        details: &mut SipITestDetails,
    ) -> SipITestResult {
        ...
        match self
            .send_and_analyze_sipi_response(&sip_invite, &["100"])
            .await
        {
            Ok((response, analysis)) => {
                ...
                let calling_correct =
                    analysis.calling_number_extracted.as_ref() == Some(&"15551234567".to_string());
                let called_correct =
                    analysis.called_number_extracted.as_ref() == Some(&"15559876543".to_string());
                let params_correct = self.validate_required_parameters(
                    &analysis.isup_parameters_found,
                    &test.required_parameters,
                );

                details.parameter_mapping_correct =
                ...
                SipITestResult {
                    ...
                    status,
                    message: format!(
                        "Parameter mapping: Calling={}, Called={}, Params={}",
                        calling_correct, called_correct, params_correct
                    ),
                    timestamp: Utc::now(),
                    details: details.clone(),
    ...
    async fn test_error_handling(
        ...
        match self
            .send_and_analyze_sipi_response(&invalid_sip, &["400", "500"])
            .await
        {
    ...
    async fn send_and_analyze_sipi_response(
        &self,
        message: &str,
        expected_codes: &[&str],
    ) -> Result<(String, SipIAnalysis)> {
        // Send message
        self.socket
            .send_to(message.as_bytes(), self.test_target)
            .await?;
    ...
    async fn analyze_sipi_response(&self, response: &str) -> SipIAnalysis {
        ...
        if analysis.isup_detected {
            if let Some(body_start) = response.find("\r\n\r\n") {
                ...
                if let Ok(isup_data) = self
                    .extract_isup_data_from_body(body, analysis.multipart_detected)
                    .await
                {
                    if let Ok(isup_message) = self.sipi_service.parse_isup_message(&isup_data) {
                        ...
                        // Extract parameters
                        analysis.isup_parameters_found =
                            isup_message.optional.iter().map(|p| p.param_type).collect();

                        // Extract phone numbers
                        analysis.calling_number_extracted =
                            self.sipi_service.extract_calling_number(&isup_message);
                        analysis.called_number_extracted =
                            self.sipi_service.extract_called_number(&isup_message);
                    }
                }
            }
        }
    ...
    fn validate_required_parameters(
        &self,
        found: &[IsupParameterType],
        required: &[IsupParameterType],
    ) -> bool {
    ...
    async fn generate_sipi_compliance_report(&self) -> Result<SipIComplianceReport> {
        ...
        let partial = results
            .iter()
            .filter(|r| matches!(r.status, TestStatus::Partial))
            .count();
        let not_implemented = results
            .iter()
            .filter(|r| matches!(r.status, TestStatus::NotImplemented))
            .count();
        let errors = results
            .iter()
        ...
        let critical_results: Vec<_> = results
            .iter()
            .filter(|r| {
                r.test_id.contains("critical")
                    || r.test_id.contains("iam")
                    || r.test_id.contains("parameter")
            })
            .collect();

        let critical_passed = critical_results
            .iter()
            .filter(|r| matches!(r.status, TestStatus::Pass))
            .count();
        ...
        let isup_stats = IsupStatistics {
            iam_messages_detected: results
                .iter()
                .filter(|r| r.details.isup_message_type == Some(IsupMessageType::IAM))
                .count(),
            acm_messages_detected: results
                .iter()
                .filter(|r| r.details.isup_message_type == Some(IsupMessageType::ACM))
                .count(),
            anm_messages_detected: results
                .iter()
                .filter(|r| r.details.isup_message_type == Some(IsupMessageType::ANM))
                .count(),
            rel_messages_detected: results
                .iter()
                .filter(|r| r.details.isup_message_type == Some(IsupMessageType::REL))
                .count(),
            calling_party_extractions: results
                .iter()
                .filter(|r| r.details.calling_number_extracted.is_some())
                .count(),
            called_party_extractions: results
                .iter()
                .filter(|r| r.details.called_number_extracted.is_some())
                .count(),
            cic_allocations: results
                .iter()
                .filter(|r| r.details.isup_cic.is_some())
                .count(),
            multipart_bodies_processed: results
                .iter()
                .filter(|r| r.details.multipart_detected)
                .count(),
            parameter_mappings_correct: results
                .iter()
                .filter(|r| r.details.parameter_mapping_correct)
                .count(),
        };
        ...
        if isup_stats.iam_messages_detected == 0 {
            recommendations.push(
                "No ISUP IAM messages detected. Implement SIP to ISUP IAM conversion.".to_string(),
            );
        }

        if isup_stats.parameter_mappings_correct < total_tests / 2 {
            recommendations.push(
                "Parameter mapping needs improvement for proper ISUP interworking.".to_string(),
            );
        }

        if failed > 0 {
            recommendations.push(format!(
                "{} SIP-I tests failed. Review ISUP implementation.",
                failed
            ));
        }
    ...
    async fn create_test_isup_message(&self, msg_type: IsupMessageType) -> IsupMessage {
        match msg_type {
            ...
            _ => {
                // Default IAM
                self.sipi_service
                    .sip_to_iam("+15551234567", "+15559876543", 123)
                    .unwrap()
            }
        }
    }
...
pub async fn run_sipi_compliance_tests(
    ...
    info!(
        "Critical compliance: {:.1}%",
        report.critical_compliance_percentage
    );
    info!(
        "ISUP messages detected: IAM={}, ACM={}, ANM={}, REL={}",

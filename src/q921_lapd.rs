...
use crate::q931_messages::{IsdnVariant, IsdnSideType, Q931Message};
...
impl LapdFrame {
    ...
    pub fn parse(data: &[u8]) -> Result<Self> {
        if data.len() < 6 { // Min: 2 addr + 1 control + 2 FCS + flags
        ...
        if unstuffed.len() < 5 { // 2 addr + 1 control + 2 FCS
...
impl Q921LapdManager {
    ...
    pub async fn process_d_channel_data(&self, data: &[u8]) -> Result<()> {
        ...
        debug!("Received LAPD frame: SAPI={}, TEI={}, Control={:?}",
        ...
        let connection = connections.entry(connection_id.clone())
        ...
        match &frame.control {
            LapdControlField::Information { n_s, n_r, p_f } => {
                self.process_i_frame(connection, *n_s, *n_r, *p_f, frame.information).await?;
            }
            LapdControlField::Supervisory { function, n_r, p_f } => {
                self.process_s_frame(connection, *function, *n_r, *p_f).await?;
            }
    ...
    pub async fn send_q931_message(&self, sapi: u8, tei: u8, message: &Q931Message) -> Result<()> {
        ...
        if connection.state != LapdState::MultipleFrameEstablished {
            return Err(anyhow!("LAPD connection not established: {}", connection_id));
        }
        ...
        info!("Sent Q.931 message over LAPD: SAPI={}, TEI={}, Message={:?}",
    ...
    async fn process_i_frame(
        ...
        if connection.state != LapdState::MultipleFrameEstablished {
            warn!("Received I-frame on non-established connection: {}", connection.connection_id);
            return Ok(());
        }
        
        // Check sequence number
        if !connection.in_receive_window(n_s) {
            warn!("I-frame sequence number out of window: {} (expected {})", n_s, connection.v_r);
            // Send REJ frame
            self.send_supervisory_frame(connection.sapi, connection.tei, 
                                      SupervisoryFunction::REJ, connection.v_r, false).await?;
            return Ok(());
        }
        ...
        if let Some(data) = information {
            if n_s == connection.v_r {
                ...
                match Q931Message::parse(&data) {
                    Ok(message) => {
                        info!("Received Q.931 message: {:?} on SAPI{}-TEI{}", 
        ...
        self.send_supervisory_frame(connection.sapi, connection.tei,
                                  SupervisoryFunction::RR, connection.v_r, false).await?;
    ...
    async fn process_u_frame(
        ...
        match function {
            UnnumberedFunction::SABME => {
                info!("Received SABME on SAPI{}-TEI{}", connection.sapi, connection.tei);
                // Send UA response
            ...
            UnnumberedFunction::DISC => {
                info!("Received DISC on SAPI{}-TEI{}", connection.sapi, connection.tei);
                // Send UA response
            ...
            UnnumberedFunction::UA => {
                debug!("Received UA on SAPI{}-TEI{}", connection.sapi, connection.tei);
            ...
            UnnumberedFunction::DM => {
                debug!("Received DM on SAPI{}-TEI{}", connection.sapi, connection.tei);
                connection.state = LapdState::TeiAssigned;
            }
            _ => {
    ...
    pub async fn establish_connection(&self, sapi: u8, tei: u8) -> Result<()> {
        ...
        let connection = connections.entry(connection_id)
    ...
    pub async fn get_statistics(&self) -> LapdStatistics {
        ...
        let established_connections = connections.values()

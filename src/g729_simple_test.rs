...
fn optimized_lsp_quantization(lsp: &[f32; 10], codebook_size: usize) -> (usize, f32) {
    ...
    for chunk_start in (0..codebook_size).step_by(4) {
        ...
        for index in chunk_start..chunk_end {
            ...
            while i + 4 <= 10 {
                let entry_val0 =
                    (index as f32 / codebook_size as f32 + (i + 0) as f32 / 10.0) * 0.1;
                let entry_val1 =
                    (index as f32 / codebook_size as f32 + (i + 1) as f32 / 10.0) * 0.1;
                let entry_val2 =
                    (index as f32 / codebook_size as f32 + (i + 2) as f32 / 10.0) * 0.1;
                let entry_val3 =
                    (index as f32 / codebook_size as f32 + (i + 3) as f32 / 10.0) * 0.1;
...
fn generate_test_signal(frames: usize) -> Vec<Vec<f32>> {
    ...
    for frame_idx in 0..frames {
        ...
        for i in 0..G729_WINDOW_SIZE {
            ...
            let window = 0.54
                - 0.46
                    * (2.0 * std::f32::consts::PI * i as f32 / (G729_WINDOW_SIZE - 1) as f32).cos();
...
fn benchmark_scalar(test_signals: &[Vec<f32>]) -> (f64, f64) {
    ...
    (
        total_autocorr_time / 1_000_000.0,
        total_lsp_time / 1_000_000.0,
    )
}
...
pub fn run_g729_performance_test() {
    ...
    println!(
        "Scalar real-time capable: {}",
        if total_scalar <= required_time_ms {
            "✓ Yes"
        } else {
            "✗ No"
        }
    ...
    println!(
        "Optimized real-time capable: {}",
        if total_opt <= required_time_ms {
            "✓ Yes"
        } else {
            "✗ No"
        }
    ...
    println!(
        "Data processed: {:.2} MB",
        (test_frames * G729_WINDOW_SIZE * 4) as f64 / 1_000_000.0
    );
    println!(
        "Scalar throughput: {:.0} frames/sec",
        test_frames as f64 / (total_scalar / 1000.0)
    );
    println!(
        "Optimized throughput: {:.0} frames/sec",
        test_frames as f64 / (total_opt / 1000.0)
    );
}

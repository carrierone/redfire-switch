...
use crate::sip_info_dtmf::{SipInfoDtmfProcessor, SipInfoDtmfMessage};
use crate::sigtran_dtmf::{SigtranDtmfProcessor, SigtranDtmfMessage};
use crate::tdmoe_ni2_signaling::{TdmoeNi2Signaling, Ni2Event, Ni2Message};
...
impl TdmoeDtmfIntegration {
    ...
    pub async fn new(ni2_signaling: Arc<TdmoeNi2Signaling>) -> Result<Self> {
        ...
        rfc2833_processor.add_payload_type(101, crate::rfc2833_events::Rfc2833PayloadType::TelephoneEvent(101));
    ...
    pub async fn add_tdm_channel(&self, config: TdmoeDtmfChannelConfig) -> Result<()> {
        ...
        stats.active_detection_channels = configs.values()
            .filter(|c| c.enable_detection)
            .count();
        stats.active_generation_channels = configs.values()
            .filter(|c| c.enable_generation)
            .count();
    ...
    pub async fn process_tdm_audio(&self, channel_id: &str, samples: &[i16]) -> Result<()> {
        ...
        let config = configs.get(channel_id)
        ...
        let buffer = audio_buffers.get_mut(channel_id)
    ...
    pub async fn generate_tdm_dtmf(&self, channel_id: &str, digit: char, duration: Option<Duration>) -> Result<Vec<i16>> {
        let configs = self.channel_configs.read().await;
        let config = configs.get(channel_id)
            .ok_or_else(|| anyhow!("TDM channel '{}' not configured for DTMF", channel_id))?;
        
        if !config.enable_generation {
            return Err(anyhow!("DTMF generation disabled for channel '{}'", channel_id));
        }
        ...
        info!("Generated DTMF digit '{}' for TDM channel '{}' ({} samples)", 
              digit, channel_id, i16_samples.len());
    ...
    pub async fn associate_sip_call(&self, tdm_channel: &str, sip_call_id: &str) -> Result<()> {
        ...
        info!("Associated TDM channel '{}' with SIP call '{}'", tdm_channel, sip_call_id);
        Ok(())
    }
    
    /// Relay DTMF from TDM to SIP via RFC2833
    pub async fn relay_dtmf_to_rfc2833(&self, tdm_channel: &str, digit: char, duration_ms: u32) -> Result<()> {
        let mappings = self.call_mappings.read().await;
        let sip_call_id = mappings.get(tdm_channel)
            .ok_or_else(|| anyhow!("No SIP call associated with TDM channel '{}'", tdm_channel))?;
        
        // Generate RFC2833 packets
        let packets = self.rfc2833_processor.generate_outgoing_packets(
            sip_call_id,
            digit,
            duration_ms,
            20, // Volume level
        ...
        info!("Generated {} RFC2833 packets for DTMF '{}' (TDM {} -> SIP {})", 
              packets.len(), digit, tdm_channel, sip_call_id);
    ...
    pub async fn relay_dtmf_from_sip(&self, sip_call_id: &str, digit: char, duration_ms: u32) -> Result<()> {
        // Find TDM channel associated with this SIP call
        let mappings = self.call_mappings.read().await;
        let tdm_channel = mappings.iter()
        ...
        let _samples = self.generate_tdm_dtmf(
            &tdm_channel, 
            digit, 
        ...
        info!("Relayed DTMF '{}' from SIP '{}' to TDM '{}'", digit, sip_call_id, tdm_channel);
        Ok(())
    }
    ...
    pub async fn process_ni2_dtmf_signaling(&self, channel_id: &str, digits: &str) -> Result<()> {
        ...
        info!("Processed NI-2 DTMF signaling for channel '{}': '{}'", channel_id, digits);
        Ok(())
    }
    ...
    pub async fn start_event_processing(&self) -> Result<()> {
        ...
        let mut dtmf_receiver = dtmf_receiver_guard.take()
        ...
        tokio::spawn(async move {
            while let Some(event) = dtmf_receiver.recv().await {
                match event {
                    DtmfEvent::DigitDetected { digit, duration, timestamp, confidence, source } => {
                    ...
                    DtmfEvent::SequenceComplete { sequence, total_duration, source } => {
                        if source == DtmfSource::TdmoeVoice {
                            let mappings = call_mappings.read().await;

    ...
    fn default() -> Self {
        Self {
            ...
            supported_translations: vec![
                CodecTranslation {
                    from: AudioCodec::G711Ulaw,
                    to: AudioCodec::G711Alaw,
                },
                CodecTranslation {
                    from: AudioCodec::G711Alaw,
                    to: AudioCodec::G711Ulaw,
                },
                CodecTranslation {
                    from: AudioCodec::G711Ulaw,
                    to: AudioCodec::G729,
...
impl CodecService {
    ...
    fn g729_encode_chunked(codec: &mut G729Codec, pcm: &[i16]) -> Result<Vec<u8>> {
        ...
        for chunk in pcm.chunks(G729_FRAME_SIZE) {
            if chunk.len() == G729_FRAME_SIZE {
                ...
                // Zero-pad the final short frame to a full 10ms frame.
    ...
    fn g729_decode_chunked(codec: &mut G729Codec, data: &[u8]) -> Result<Vec<i16>> {
        const G729_ENCODED_SIZE: usize = 10;
        if data.is_empty() {
            return Ok(Vec::new());
        }
        let mut pcm = Vec::new();
    ...
    pub async fn start_g729_annex_session(&self, session_id: String) -> Result<()> {
        if let Some(ref processor) = self.g729_annex_processor {
            ...
            let mut g729_codecs = self.g729_codecs.lock().await;
            g729_codecs.insert(session_id, G729Codec::new());
            Ok(())
        }
    }

...
impl AniIIInfo {
    ...
    pub fn from_oli_info(oli: OriginatingLineInfo) -> Option<Self> {
        ...
        Some(AniIIInfo {
            ...
            is_payphone: matches!(
                code,
                AniIICode::PayStationNetworkCoin
                    | AniIICode::PayStationNonNetworkCoin
                    | AniIICode::CoinNonCoinUncertainty
            ),
    ...
    pub fn calculate_surcharge(
        ...
        if let Some(info) = ani_ii {
            ...
            let surcharge = if let Some(config) = trunk_config {
                config
                    .get(&format!("surcharge_{}", info.raw_digit))
                    .copied()
            ...
            (
                true,
                surcharge,
                format!("Payphone surcharge for ANI-II {}", info.raw_digit),
            )
        } else {
...
mod tests {
    ...
    fn test_surcharge_calculation() {
        ...
        let (applies, amount, reason) =
            RfcCompliantAniIIParser::calculate_surcharge(Some(&ani_ii), true, None);

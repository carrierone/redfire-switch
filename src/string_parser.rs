...
pub fn parse_escape_sequence<'a>(input: &mut &'a str) -> ModalResult<char> {
    preceded(
        '\\',
        alt((
            'n'.value('\n'),
...
pub fn parse_quoted_string<'a>(input: &mut &'a str) -> ModalResult<String> {
    delimited(
        '"',
        repeat(0.., alt((
...
pub fn parse_config_line<'a>(input: &mut &'a str) -> ModalResult<(String, String)> {
    let key = terminated(
...
pub fn parse_timer_config<'a>(input: &mut &'a str) -> ModalResult<HashMap<String, u32>> {
    let mut timers = HashMap::new();
    
    let entries: Vec<_> = repeat(0.., (
...
pub fn parse_isdn_config_section<'a>(input: &mut &'a str) -> ModalResult<IsdnConfigSection> {
    let name = delimited('[', take_while(1.., |c: char| c != ']'), ']').parse_next(input)?;
    
    let entries: Vec<(String, String)> = repeat(0..,
        preceded(
            space0,
            parse_config_line,
        ),
...
pub fn parse_codec_type<'a>(input: &mut &'a str) -> ModalResult<String> {
    alt((

...
async fn main() {
    println!(
        "{}",
        "🧪 Phone Number Validation & International Routing Test Suite"
            .cyan()
            .bold()
    );
    ...
    let test_categories = vec![
        ...
        (
            ...
            test_routing_integration().await,
        ),
        (
...
async fn test_phone_validation() -> Vec<(String, TestResult)> {
    ...
    results.push((
        ...
        run_test(|| {
            ...
            assert!(
                !result.is_valid,
                "Invalid number should fail strict validation"
            );
    ...
    results.push((
        ...
        run_test(|| {
            ...
            let row_countries = vec![
                "US", "CA", "MX", "AU", "NZ", "JP", "KR", "CN", "IN", "BR", "AR", "CL", "ZA", "RU",
                "TR", "AE", "SA",
            ];

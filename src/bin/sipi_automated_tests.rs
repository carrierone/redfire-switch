...
use redfire_switch::sipt_sipi::{
    ...
    IsupParameterType, IsupParameter, IsupVariant
};
...
impl SipIAutomatedTester {
    ...
    async fn run_all_tests(&mut self) -> Result<()> {
        ...
        println!("{}", "🔥 SIP-I AUTOMATED TESTING SUITE 🔥".bright_red().bold());
    ...
    async fn test_iam_encoding(&self) -> Result<()> {
        let iam = self.sipi_service.sip_to_iam("+15551234567", "+15559876543", 100)?;
...
"#;
        ...
        let iam = self.sipi_service.sip_to_iam("+15551234567", "+15559876543", 500)?;
    ...
    async fn test_phone_number_mapping(&self) -> Result<()> {
        ...
        let test_numbers = vec![
            ...
            ("+8612345678901", vec![0x86, 0x12, 0x34, 0x56, 0x78, 0x90, 0x01]),
        ];
    ...
    async fn test_international_numbers(&self) -> Result<()> {
        let iam = self.sipi_service.sip_to_iam("+442071234567", "+8612345678901", 100)?;
    ...
    async fn test_cic_release(&self) -> Result<()> {
        ...
        // Create and release
        let _iam = self.sipi_service.sip_to_iam("+15551234567", "+15559876543", cic)?;
    ...
    async fn test_sipi_call_flow(&self) -> Result<()> {
        ...
        let iam = self.sipi_service.sip_to_iam("+15551234567", "+15559876543", cic)?;
    ...
    async fn test_buffer_overflow_protection(&self) -> Result<()> {
        ...
        match self.sipi_service.sip_to_iam(&huge_number, "+15559999999", 1) {
            Ok(_) => {
    ...
    async fn test_rate_limiting(&self) -> Result<()> {
        ...
        for i in 0..100 {
            let _ = self.sipi_service.sip_to_iam(
                "+15551234567",
                "+15559876543",
                (i % 1000) as u16 + 1
            );
    ...
    fn display_test_results(&self) {
        ...
        println!("\n{}", "═".repeat(70).bright_yellow());
        if failed == 0 {
            println!("{}", "🎉 ALL TESTS PASSED! SIP-I IMPLEMENTATION VERIFIED! 🎉".green().bold());
        } else {
            println!("{}", "⚠️ SOME TESTS FAILED - REVIEW REQUIRED ⚠️".yellow().bold());
        }
        println!("{}", "═".repeat(70).bright_yellow());
    }
}

...
impl CoreTestSuite {
        ...
        match UdpSocket::bind("127.0.0.1:0").await {
            Ok(socket) => {
                match socket.local_addr() {
                    ...
                    Err(e) => {
                        result = result.fail(start.elapsed(), &format!("Failed to get socket address: {}", e));
                    }
                }
            }
            Err(e) => {
    ...
    async fn test_sip_message_validation(&mut self) {
        ...
        if passed_tests == 4 {
            ...
            result = result.fail(start.elapsed(), &format!("{}/4 validation tests passed", passed_tests));
        }
    ...
    async fn test_call_id_extraction(&mut self) {
        ...
        match call_id {
            ...
            Err(e) => {
                result = result.fail(start.elapsed(), &format!("Failed to extract Call-ID: {}", e));
            }
        }
    ...
    async fn test_udp_messaging(&mut self) {
        ...
        match self.test_udp_echo().await {
            Ok(duration) => {
                result = result.pass(start.elapsed(), &format!("UDP echo completed in {:?}", duration));
            }
            Err(e) => {
    ...
    async fn test_concurrent_processing(&mut self) {
        ...
        if completed == 10 {
            result = result.pass(start.elapsed(), "All concurrent tasks completed successfully");
        } else {
            result = result.fail(start.elapsed(), &format!("Only {}/10 concurrent tasks completed", completed));
        }
    ...
    async fn test_performance_baseline(&mut self) {
        ...
        if ops_per_second > 10000.0 {
            result = result.pass(start.elapsed(), &format!("{:.0} ops/sec - Good performance", ops_per_second));
        } else {
            result = result.fail(start.elapsed(), &format!("{:.0} ops/sec - Below baseline", ops_per_second));
        }
    ...
    async fn test_division_by_zero_protection(&mut self) {
        ...
        if passed_tests == 3 {
            ...
            result = result.fail(start.elapsed(), &format!("{}/3 division tests passed", passed_tests));
        }
    ...
    async fn test_memory_leak_prevention(&mut self) {
        ...
        let now = Instant::now();
        call_sessions.retain(|_id, created_at| {
            now.duration_since(*created_at) < Duration::from_micros(5)
        });
        ...
        if cleaned_count > 0 {
            result = result.pass(start.elapsed(), &format!("Cleaned {} sessions, {} remain", cleaned_count, final_count));
        } else {
    ...
    async fn test_malicious_input_handling(&mut self) {
        ...
        if handled_safely == 6 {
            ...
            result = result.fail(start.elapsed(), &format!("{}/6 malicious inputs handled safely", handled_safely));
        }

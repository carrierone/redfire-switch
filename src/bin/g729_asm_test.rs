...
use redfire_switch::g729_external_asm::{
    ExternalAsmG729Codec, autocorrelation_optimized, levinson_durbin_optimized, L_WINDOW
};
...
fn test_codec() {
    ...
    println!("  CPU features - SSE: {}, AVX: {}", stats.cpu_features.sse, stats.cpu_features.avx);
    
    // Real-time test
    let real_time_capable = duration.as_millis() <= 10;
    println!("  Real-time capable: {} ({}ms <= 10ms)", 

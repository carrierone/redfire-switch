...
use redfire_switch::tdmoe_ni2_signaling::{Ni2CallState, Ni2Event, Ni2SideType, TdmoeNi2Signaling};
    ...
    async fn monitor_events(&mut self) {
        while let Ok(event) = self.event_receiver.recv().await {
            ...
            match event {
                ...
                Ni2Event::CallDisconnected {
                    channel_id,
                    call_reference,
                    cause,
                } => {
                    info!(
                        "❌ [{}] Call Disconnected: {} (CRV: {}, cause: {})",
                        self.side_name, channel_id, call_reference, cause
                    );
                }
                Ni2Event::CallStateChanged {
                    channel_id,
                    old_state,
                    new_state,
                } => {
                ...
                Ni2Event::InformationElementSent {
                    channel_id,
                    element,
                } => {
                    info!(
...
impl Ni2CallTest {
    ...
    async fn run_call_flow_test(&self) -> Result<()> {
        ...
        info!("\n=== STEP 3: Network Side Sends CALL PROCEEDING ===");
        self.network_side
            .send_call_proceeding(&self.test_channel)
            .await?;
        ...
        info!("\n=== STEP 6: User Side Disconnects Call ===");
        self.user_side
            .send_disconnect(&self.test_channel, 16)
            .await?; // Normal call clearing
    ...
    async fn test_side_type_enforcement(&self) -> Result<()> {
        ...
        match self
            .network_side
            .initiate_call("T1-1-2", "15551111111", "15552222222")
            .await
        {

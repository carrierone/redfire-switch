...
use redfire_switch::cesopsn::{
    CesopsnManager, CesopsnCircuitConfig, CesopsnCircuitType, 
    CesopsnServiceQuality, CesopsnPayloadType
};
use redfire_switch::q931_messages::{IsdnConfig, IsdnVariant, IsdnSideType};
use redfire_switch::cesopsn::{PcmCodec};
use redfire_switch::cesopsn_ni2_integration::{
    CesopsnNi2Integration, CesopsnNi2CircuitConfig, CesopsnNi2Event
};
...
impl CesopsnTestEnvironment {
    ...
    async fn initialize(&mut self) -> Result<()> {
        ...
        self.network_side.add_circuit(self.network_config.clone()).await?;
        
        info!("✅ User side circuit {} configured on {}", 
              self.user_config.cesopsn_config.circuit_id,
              self.user_config.cesopsn_config.local_address);
        
        info!("✅ Network side circuit {} configured on {}", 
    ...
    async fn test_dtmf_transmission(&self) -> Result<()> {
        ...
        for digit in test_digits.chars() {
            info!("🎵 User side: Generating DTMF '{}' on channel {}", digit, channel);
            
            // User side generates DTMF
            if let Err(e) = self.user_side.generate_dtmf(
                self.user_config.cesopsn_config.circuit_id,
                channel,
                digit,
                150 // 150ms duration
    ...
    async fn test_circuit_monitoring(&self) -> Result<()> {
        ...
        if let Ok(user_stats) = self.user_side.get_circuit_stats(
            self.user_config.cesopsn_config.circuit_id
        ).await {
            info!("📊 User Side Circuit {} Statistics:", user_stats.circuit_id);
            info!("   NI-2 Active Calls: {}", user_stats.ni2_active_calls);
            info!("   DTMF Events Detected: {}", user_stats.dtmf_events_detected);
            info!("   DTMF Events Generated: {}", user_stats.dtmf_events_generated);
        }
        
        // Get network side statistics  
        if let Ok(network_stats) = self.network_side.get_circuit_stats(
            self.network_config.cesopsn_config.circuit_id
        ).await {
            info!("📊 Network Side Circuit {} Statistics:", network_stats.circuit_id);
            info!("   NI-2 Active Calls: {}", network_stats.ni2_active_calls);
            info!("   DTMF Events Detected: {}", network_stats.dtmf_events_detected);
            info!("   DTMF Events Generated: {}", network_stats.dtmf_events_generated);
        }
    ...
    async fn test_qos_handling(&self) -> Result<()> {
        ...
        info!("   Service Quality: {:?}", self.user_config.cesopsn_config.service_quality);
        info!("   Jitter Buffer: {}ms", self.user_config.cesopsn_config.jitter_buffer_ms);
        info!("   Frames per Packet: {}", self.user_config.cesopsn_config.frames_per_packet);
        info!("   Adaptive Clock Recovery: {}", self.user_config.cesopsn_config.enable_acr);
        ...
        for i in 0..10 {
            ...
            info!("   Packet {} sent with {}ms simulated delay", i + 1, delay_ms);
        }
    ...
    async fn start_event_monitoring(&self) {
        ...
        tokio::spawn(async move {
            while let Ok(event) = user_events.recv().await {
                match event {
                    CesopsnNi2Event::DtmfDetected { circuit_id, channel, digit, duration, confidence } => {
                        info!("🎵 [USER] DTMF Detected: '{}' on C{}-{} ({}ms, conf: {:.2})", 
                              digit, circuit_id, channel, duration.as_millis(), confidence);
                    }
                    CesopsnNi2Event::DtmfGenerated { circuit_id, channel, digit, duration } => {
                        info!("🎼 [USER] DTMF Generated: '{}' on C{}-{} ({}ms)", 
                              digit, circuit_id, channel, duration.as_millis());
                    }
                    CesopsnNi2Event::CircuitStateChanged { circuit_id, old_state, new_state } => {
                        info!("🔄 [USER] Circuit {} state: {} -> {}", circuit_id, old_state, new_state);
                    }
                    CesopsnNi2Event::Ni2MessageReceived { circuit_id, channel_id, message } => {
                        info!("📨 [USER] NI-2 Message on {}: {} bytes", channel_id, message.len());
                    }
                    CesopsnNi2Event::QualityDegraded { circuit_id, loss_rate, jitter_ms } => {
                        warn!("⚠️  [USER] Circuit {} quality degraded: {:.1}% loss, {:.1}ms jitter", 
                              circuit_id, loss_rate * 100.0, jitter_ms);
                    }
                }
            }
        });
        ...
        tokio::spawn(async move {
            while let Ok(event) = network_events.recv().await {
                match event {
                    CesopsnNi2Event::DtmfDetected { circuit_id, channel, digit, duration, confidence } => {
                        info!("🎵 [NET] DTMF Detected: '{}' on C{}-{} ({}ms, conf: {:.2})", 
                              digit, circuit_id, channel, duration.as_millis(), confidence);
                    }
                    CesopsnNi2Event::DtmfGenerated { circuit_id, channel, digit, duration } => {
                        info!("🎼 [NET] DTMF Generated: '{}' on C{}-{} ({}ms)", 
                              digit, circuit_id, channel, duration.as_millis());
                    }
                    CesopsnNi2Event::CircuitStateChanged { circuit_id, old_state, new_state } => {
                        info!("🔄 [NET] Circuit {} state: {} -> {}", circuit_id, old_state, new_state);
                    }
                    CesopsnNi2Event::Ni2MessageReceived { circuit_id, channel_id, message } => {
                        info!("📨 [NET] NI-2 Message on {}: {} bytes", channel_id, message.len());
                    }
                    CesopsnNi2Event::QualityDegraded { circuit_id, loss_rate, jitter_ms } => {
                        warn!("⚠️  [NET] Circuit {} quality degraded: {:.1}% loss, {:.1}ms jitter", 
                              circuit_id, loss_rate * 100.0, jitter_ms);
                    }
                }
            }

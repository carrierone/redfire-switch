...
impl OptimizedCodecProcessor {
    ...
    pub unsafe fn ulaw_to_linear_batch_avx2(&self, input: &[u8], output: &mut [i16]) {
        ...
        for i in 0..chunks {
            ...
            for j in 0..16 {
                ...
                *output.get_unchecked_mut(offset + j) = *self.ulaw_to_linear_table.get_unchecked(ulaw_val as usize);
            }
        }
    }
    ...
    pub unsafe fn alaw_to_linear_batch_avx2(&self, input: &[u8], output: &mut [i16]) {
        ...
        for i in 0..(input.len() / 16) {
            ...
            for j in 0..16 {
                ...
                *output.get_unchecked_mut(offset + j) = *self.alaw_to_linear_table.get_unchecked(alaw_val as usize);
            }
        }
    }
    ...
    fn linear_to_ulaw_slow(&self, linear: i16) -> u8 {
        ...
        let mantissa = if exponent == 0 {
            if magnitude >= 33 { (magnitude - 33) >> 2 } else { 0 }
        } else {
            if magnitude >= 33 { (magnitude - 33) >> (exponent + 2) } else { 0 }
        } & 0x0F;
...
mod tests {
    ...
    fn test_codec_processor_tables() {
        ...
        for ulaw_val in 0..=255u8 {
            ...
            let tolerance = if ulaw_val == 0 || ulaw_val == 255 { 255 } else { 8 };
            let diff = (ulaw_val as i16 - back_to_ulaw as i16).abs();
            assert!(
                diff <= tolerance,
                "µ-Law round-trip failed for {}: {} -> {} -> {} (diff: {}, tolerance: {})",
                ulaw_val, ulaw_val, linear, back_to_ulaw, diff, tolerance
            );
        }
        ...
        for alaw_val in 0..=255u8 {
            ...
            let tolerance = if alaw_val == 0 || alaw_val == 255 { 255 } else { 8 };
            let diff = (alaw_val as i16 - back_to_alaw as i16).abs();
            assert!(
                diff <= tolerance,
                "A-Law round-trip failed for {}: {} -> {} -> {} (diff: {}, tolerance: {})",
                alaw_val, alaw_val, linear, back_to_alaw, diff, tolerance
            );
        }
    }

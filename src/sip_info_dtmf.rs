                ...
                match key.as_str() {
                    ...
                    "volume" => {
                        if let Ok(volume) = value.parse::<u32>() {
                            ...
                            let normalized_volume = if matches!(content_type, SipInfoDtmfContentType::CiscoDtmfRelay) {
                                (volume * 100) / 63
                            } else {
                                volume
                            };
...
impl SipInfoDtmfProcessor {
    ...
    pub async fn process_incoming_info(
        ...
        debug!("Processing SIP INFO DTMF for session {}: {}", session_id, content_type);
        ...
        self.update_session(session_id, call_id, from_tag, to_tag, dtmf_message.digit).await;
        
        // Generate DTMF event
        let duration = dtmf_message.duration
            .map(|d| Duration::from_millis(d.into()))
            .unwrap_or(Duration::from_millis(100)); // Default 100ms
        
        let confidence = dtmf_message.volume
        ...
        if let Err(e) = self.event_sender.send(dtmf_event) {
            ...
            info!("SIP INFO DTMF '{}' received for session {} (duration: {:?})", 
    ...
    pub async fn generate_outgoing_info(
        ...
        info!("Generated SIP INFO DTMF '{}' for session {}", digit, session_id);
        Ok(request)
    }
    ...
    async fn update_session(&self, session_id: &str, call_id: &str, from_tag: &str, 
                           to_tag: &str, digit: char) {
...
mod tests {
    ...
    fn test_sip_info_message_cisco_format() {
        ...
        // Test round-trip
        let parsed = SipInfoDtmfMessage::from_body_content("application/dtmf-relay", &body).unwrap();
    ...
    fn test_sip_info_message_generic_format() {
        let message = SipInfoDtmfMessage::new('*', SipInfoDtmfContentType::GenericDtmf)
            .with_duration(100);
    ...
    fn test_sip_info_message_nortel_format() {
        let message = SipInfoDtmfMessage::new('#', SipInfoDtmfContentType::NortelText)
            .with_duration(150);
        ...
        // Test parsing
        let parsed = SipInfoDtmfMessage::from_body_content("application/vnd.nortel.text", &body).unwrap();
        assert_eq!(parsed.digit, '#');

...
impl SipMessageValidator {
    ...
    fn validate_start_line(&self, line: &str) -> Result<(), SecurityError> {
        if line.starts_with("SIP/2.0") {
            ...
            if parts.len() < 3 {
                return Err(SecurityError::InvalidInput("Invalid response line format".to_string()));
            }
            
            // Validate status code
            if let Err(_) = parts[1].parse::<u16>() {
                return Err(SecurityError::InvalidInput("Invalid status code".to_string()));
            }
        } else {
            // Request line: <method> <request-uri> SIP/2.0
            let parts: Vec<&str> = line.split_whitespace().collect();
            if parts.len() != 3 {
                return Err(SecurityError::InvalidInput("Invalid request line format".to_string()));
            }
            
            // Validate method
            if !self.valid_methods.contains(&parts[0].to_string()) {
                warn!("Unknown SIP method: {}", parts[0]);
                return Err(SecurityError::InvalidInput(format!("Invalid SIP method: {}", parts[0])));
            }
            
            // Validate SIP version
            if parts[2] != "SIP/2.0" {
                return Err(SecurityError::InvalidInput("Invalid SIP version".to_string()));
            }
            
            // Basic URI validation
            if !parts[1].starts_with("sip:") && !parts[1].starts_with("sips:") {
                return Err(SecurityError::InvalidInput("Invalid SIP URI scheme".to_string()));
            }
        }
    ...
    fn validate_header_line(&self, line: &str) -> Result<(), SecurityError> {
        ...
        if line.contains('\r') || line.contains('\n') {
            return Err(SecurityError::InvalidInput("Header injection detected".to_string()));
        }
        
        // Parse header
        let header_parts: Vec<&str> = line.splitn(2, ':').collect();
        if header_parts.len() != 2 {
            return Err(SecurityError::InvalidInput("Invalid header format".to_string()));
        }
        ...
        if header_name.is_empty() || header_name.len() > 100 {
            return Err(SecurityError::InvalidInput("Invalid header name".to_string()));
        }
        
        // Validate header value length
        if header_value.len() > 4096 {
            return Err(SecurityError::InvalidInput("Header value too long".to_string()));
        }
        ...
        if let Some(pattern) = self.header_patterns.get(header_name) {
            if !pattern.is_match(header_value) {
                return Err(SecurityError::InvalidInput(format!("Invalid {} header format", header_name)));
            }
        }
        ...
        for line in sdp.lines() {
            if !line.is_empty() {
                ...
                if line.len() < 3 || !line.chars().nth(1).map_or(false, |c| c == '=') {
                    return Err(SecurityError::InvalidInput("Invalid SDP format".to_string()));
                }
                
                // Check for dangerous characters
                if line.contains("$(") || line.contains("`") || line.contains("${") {
                    return Err(SecurityError::InvalidInput("Dangerous characters in SDP".to_string()));
                }
            }
        }
...
pub fn validate_phone_number(number: &str) -> Result<String, SecurityError> {
    ...
    if cleaned.starts_with('+') {
        if cleaned.len() < 8 || cleaned.len() > 15 {
            return Err(SecurityError::InvalidInput("Invalid E.164 number format".to_string()));
        }
    } else if cleaned.len() < 7 || cleaned.len() > 15 {
        return Err(SecurityError::InvalidInput("Invalid phone number length".to_string()));
    }
    ...
    match addr {
        std::net::IpAddr::V4(ipv4) => {
            ...
            if octets[0] == 127 {
                ...
                return Err(SecurityError::InvalidInput("Localhost IP not allowed".to_string()));
            }
            // Block multicast (224.0.0.0/4)
            if octets[0] >= 224 && octets[0] <= 239 {
                return Err(SecurityError::InvalidInput("Multicast IP not allowed".to_string()));
            }
        }

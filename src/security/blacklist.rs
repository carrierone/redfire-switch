...
impl BlacklistManager {
        ...
        if let Some(trunk_config) = trunk_configs.get(trunk_id) {
            ...
            if let Some(score) = trunk_config.reputation_overrides.get(&ip) {
                if *score < self.config.auto_blacklist_threshold {
                    warn!(
                        "IP {} blocked by trunk {} reputation override: {}",
                        ip, trunk_id, score
                    );
                    return Ok(Some(false));
                }
    ...
    pub async fn add_to_blacklist(
        ...
        let expires_at = duration_seconds
            .map(|duration| chrono::Utc::now() + chrono::Duration::seconds(duration as i64));
        ...
        info!(
            "Added IP {} to blacklist: {} (source: {}, severity: {})",
    ...
    pub async fn update_reputation(&self, ip: IpAddr, success: bool, source: String) -> Result<()> {
        ...
        debug!(
            "Updated reputation for {}: score={:.3}, success_rate={:.3}, interactions={}",
        ...
        if should_blacklist {
            self.add_to_blacklist(
                ip,
                format!(
                    "Auto-blacklisted due to low reputation: {:.3}",
                    blacklist_score
                ),
    ...
    pub async fn configure_trunk(&self, trunk_config: TrunkSecurityConfig) -> Result<()> {
        ...
        info!(
            "Updated security configuration for trunk: {}",
            trunk_config.trunk_id
        );
        Ok(())
    }
    ...
    pub async fn is_feature_enabled_for_trunk(&self, feature: &str, trunk_id: &str) -> bool {
        ...
        if let Some(trunk_config) = configs.get(trunk_id) {
            match feature {
                "blacklist" => trunk_config
                    .override_blacklist
                    .unwrap_or(self.config.enable_global),
                "reputation_scoring" => trunk_config
                    .override_reputation_scoring
                    .unwrap_or(self.config.enable_reputation_scoring),
    ...
    pub async fn get_blacklist_stats(&self) -> Result<HashMap<String, u64>> {
        ...
        stats.insert(
            "total_reputation_entries".to_string(),
            reputation.len() as u64,
        );
    ...
    pub async fn import_external_blacklist(
        &self,
        ips: Vec<IpAddr>,
        source: String,
    ) -> Result<usize> {
        ...
        info!(
            "Imported {} IPs from external blacklist: {}",
            imported, source
        );
        Ok(imported)
    }
...
mod tests {
    ...
    async fn test_trunk_specific_rules() {
        ...
        assert!(manager
            .is_ip_allowed(test_ip, Some("trunk1"))
            .await
            .unwrap());
    }
    ...
    async fn test_reputation_system() {
        ...
        for _ in 0..10 {
            manager
                .update_reputation(test_ip, true, "test".to_string())
                .await
                .unwrap();
        }
        ...
        for _ in 0..50 {
            manager
                .update_reputation(test_ip, false, "test".to_string())
                .await
                .unwrap();
        }

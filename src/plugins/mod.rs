...
impl PluginManager {
    ...
    pub async fn load_plugins_from_config(&self, config_path: &Path) -> Result<()> {
        ...
        info!(
            "Loaded {} plugins from configuration",
            self.get_plugin_count().await
        );
        Ok(())
    }
    ...
    pub async fn load_plugin(&self, config: PluginConfig) -> Result<()> {
        ...
        registry
            .register_plugin(config.name.clone(), plugin_mut)
            .await?;
    ...
    ) -> Result<Vec<PluginExecutionResult>> {
            ...
            if let Some(plugin) = registry.get_plugin(&plugin_name).await {
                ...
                let result = match plugin.handle_message(message, &context).await {
                    Ok(response) => {
                        ...
                        // Update statistics
                        self.update_plugin_stats(&plugin_name, true, execution_time, None)
                            .await;
    ...
    async fn update_plugin_stats(
        ...
        plugin_stats.average_execution_time_ms =
            (plugin_stats.average_execution_time_ms * (total - 1.0) + execution_time_ms as f64)
                / total;
...
mod tests {
    ...
    async fn test_plugin_stats_update() {
        ...
        manager
            .update_plugin_stats("test-plugin", true, 100, None)
            .await;

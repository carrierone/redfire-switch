...
use std::sync::{Arc, Weak, Mutex, RwLock, MutexGuard, RwLockReadGuard, RwLockWriteGuard};
...
impl<T> SafeArc<T> {
    ...
    pub fn safe_clone(&self) -> Self {
        debug!("Cloning Arc - current strong count: {}", Arc::strong_count(&self.inner));
        Self {
    ...
    pub fn safe_read(&self, timeout: Duration) -> Result<RwLockReadGuard<T>> {
        ...
        loop {
            match self.inner.try_read() {
                Ok(guard) => {
                    debug!("Acquired read lock '{}' after {:?}", self.name, start.elapsed());
                    return Ok(guard);
                },
                Err(_) if start.elapsed() < timeout => {
                    std::thread::sleep(Duration::from_millis(1));
                ...
                Err(_) => {
                    error!("Read lock timeout after {:?} for RwLock '{}'", timeout, self.name);
    ...
    pub fn safe_write(&self, timeout: Duration) -> Result<RwLockWriteGuard<T>> {
        ...
        loop {
            match self.inner.try_write() {
                Ok(guard) => {
                    debug!("Acquired write lock '{}' after {:?}", self.name, start.elapsed());
                    return Ok(guard);
                },
                Err(_) if start.elapsed() < timeout => {
                    std::thread::sleep(Duration::from_millis(1));
                ...
                Err(_) => {
                    error!("Write lock timeout after {:?} for RwLock '{}'", timeout, self.name);
...
impl MemoryTracker {
    ...
    pub fn get_allocations(&self) -> Result<Vec<(String, usize, Duration)>> {
        ...
        Ok(allocs.iter().map(|(name, (size, time))| {
            (name.clone(), *size, now.duration_since(*time))
        }).collect())
    }
...
pub mod concurrency {
    ...
    impl LockOrderManager {
        ...
        pub fn validate_order(&self, current_locks: &[&str], new_lock: &str) -> Result<bool> {
            ...
            let new_lock_id = locks.get(new_lock).copied()
            ...
            for &current_lock in current_locks {
                let current_id = locks.get(current_lock).copied()
...
mod tests {
    ...
    fn test_safe_parsing() {
        ...
        assert_eq!(parse_duration_millis("500").unwrap(), Duration::from_millis(500));
    }
}
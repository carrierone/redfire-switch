...
use crate::q931_messages::{Q931Message, Q931MessageType, IsdnVariant, IsdnSideType, IsdnConfig, CauseValue};
use crate::q921_lapd::{Q921LapdManager, LapdEvent, LapdStatistics};
use crate::pri_timers::{PriTimerManager, PriTimerType, PriTimerAction, PriTimerExpiredEvent, ActivePriTimer, clear_call_action, send_message_action};
use crate::cesopsn_ni2_integration::{CesopsnNi2Integration, CesopsnNi2CircuitConfig, CesopsnNi2Event};
...
impl IsdnStackManager {
    ...
    pub async fn new(config: IsdnStackConfig) -> Result<Self> {
        ...
        let cesopsn_integration = Arc::new(tokio::sync::Mutex::new(CesopsnNi2Integration::new().await?));
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        for (&circuit_id, circuit_config) in &self.config.circuits {
            self.initialize_circuit(circuit_id, circuit_config.clone()).await?;
        }
    ...
    async fn initialize_circuit(&self, circuit_id: u16, config: CesopsnNi2CircuitConfig) -> Result<()> {
        info!("Initializing ISDN circuit {}: {}", circuit_id, config.description);
        
        // Add CESoPSN circuit - use safe reference instead of dangerous try_unwrap
        self.cesopsn_integration.lock().await.add_circuit(config.clone()).await?;
        ...
        self.lapd_managers.write().await.insert(circuit_id, lapd_manager);
        
        // Initialize call tracking for this circuit
        self.active_calls.write().await.insert(circuit_id, HashMap::new());
        
        // Set initial health status
        self.health_monitor.write().await.insert(circuit_id, HealthStatus::Healthy);
        
        // Track circuit allocation for memory monitoring  
        self.memory_tracker.track_allocation(&format!("circuit_{}", circuit_id), 1024)?;
    ...
    async fn start_event_processing(&self) -> Result<()> {
        ...
        for (&circuit_id, lapd_manager) in lapd_managers.iter() {
            ...
            self.start_lapd_event_processing(circuit_id, lapd_events).await;
        }
    ...
    /// Process CESoPSN events
    async fn start_cesopsn_event_processing(&self, mut events: broadcast::Receiver<CesopsnNi2Event>) {
        ...
        tokio::spawn(async move {
            async {
                ...
                match event {
                    CesopsnNi2Event::DtmfDetected { circuit_id, channel, digit, duration: _, confidence } => {
                    ...
                    CesopsnNi2Event::CircuitStateChanged { circuit_id, old_state, new_state } => {
    ...
    /// Process timer expiration events
    async fn start_timer_event_processing(&self, mut events: broadcast::Receiver<PriTimerExpiredEvent>) {
            ...
            async {
                ...
                match timer.action {
                    ...
                    PriTimerAction::SendMessage(message_type) => {
                        info!("Timer {} expired - should send {:?} message", 
                              timer.timer_type.description(), message_type);
    ...
    async fn start_lapd_event_processing(&self, circuit_id: u16, mut events: broadcast::Receiver<LapdEvent>) {
        let active_calls = Arc::clone(&self.active_calls);
        ...
        tokio::spawn(async move {
            async {
                ...
                match lapd_event {
                    LapdEvent::DataReceived { sapi: _, tei: _, data } => {
                        // Parse Q.931 message
                        match Q931Message::parse(&data) {
                            ...
                            Err(e) => {
                                warn!("Failed to parse Q.931 message on circuit {}: {}", circuit_id, e);
                            }
                        }
                    ...
                    LapdEvent::LinkEstablished { sapi, tei } => {
                        info!("LAPD link established on circuit {}: SAPI {} TEI {}", circuit_id, sapi, tei);
                    }
                    LapdEvent::LinkReleased { sapi, tei } => {
                        info!("LAPD link released on circuit {}: SAPI {} TEI {}", circuit_id, sapi, tei);
                    }
                    LapdEvent::Error { sapi, tei, error } => {
                        warn!("LAPD error on circuit {} SAPI {} TEI {}: {}", circuit_id, sapi, tei, error);
    ...
    async fn process_q931_message(
        ...
        debug!("Processing Q.931 {:?} message for CRV {} on circuit {}", 
        ...
        match message.message_type {
            Q931MessageType::Setup => {
                ...
                if let Some(calling_ie) = message.find_ie(crate::q931_messages::InformationElementType::CallingPartyNumber) {
                    // Parse calling party number
                    call.calling_number = String::from_utf8_lossy(&calling_ie.data).to_string();
                }
                if let Some(called_ie) = message.find_ie(crate::q931_messages::InformationElementType::CalledPartyNumber) {
                ...
                if let Ok(timer_id) = timer_manager.start_timer(
                    PriTimerType::T303,
                    call_ref,
                    circuit_id,
                    None,
    ...
    async fn assess_circuit_health(
        ...
        let circuit_stats = match cesopsn_integration.lock().await.get_circuit_stats(circuit_id).await {
            Ok(stats) => stats,
        ...
        if let Some(cesopsn_stats) = circuit_stats.cesopsn_stats.get(&circuit_id) {
            if cesopsn_stats.loss_rate > 0.05 { // > 5% loss
                return HealthStatus::Critical;
            } else if cesopsn_stats.loss_rate > 0.01 { // > 1% loss
        ...
        let circuit_timers: Vec<_> = active_timers.iter()
        ...
        let expired_timers = circuit_timers.iter()
    ...
    async fn get_circuit_status(&self, circuit_id: u16) -> Result<CircuitStatus> {
        let config = self.config.circuits.get(&circuit_id)
        ...
        let circuit_calls = active_calls.get(&circuit_id).map(|calls| calls.len()).unwrap_or(0);
        ...
        let circuit_timers: Vec<_> = all_timers.iter()
            .filter(|timer| timer.circuit_id == circuit_id)
            .collect();
        ...
        let cesopsn_stats = self.cesopsn_integration.lock().await.get_circuit_stats(circuit_id).await
    ...
    pub async fn shutdown(&self) -> Result<()> {
        ...
        for circuit_calls in active_calls.values() {
            for call in circuit_calls.values() {
                self.timer_manager.cancel_call_timers(call.call_reference).await;
            }
        }

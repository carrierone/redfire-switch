...
use rsip::prelude::*;
use crate::fail2ban::{Fail2BanService, FailureType};
...
impl DigestCredentials {
    ...
    fn calculate_digest_response(
        ...
        let ha1 = match algorithm {
            DigestAlgorithm::MD5 => format!("{:x}", md5::compute(format!("{}:{}:{}", self.username, realm, self.password_hash))),
            DigestAlgorithm::SHA256 => format!("{:x}", sha2::Sha256::digest(format!("{}:{}:{}", self.username, realm, self.password_hash).as_bytes())),
        };
        
        let ha2 = match algorithm {
            DigestAlgorithm::MD5 => format!("{:x}", md5::compute(format!("INVITE:{}", uri))),
            DigestAlgorithm::SHA256 => format!("{:x}", sha2::Sha256::digest(format!("INVITE:{}", uri).as_bytes())),
        };
        
        let response = match algorithm {
            DigestAlgorithm::MD5 => format!("{:x}", md5::compute(format!("{}:{}:{}", ha1, nonce, ha2))),
            DigestAlgorithm::SHA256 => format!("{:x}", sha2::Sha256::digest(format!("{}:{}:{}", ha1, nonce, ha2).as_bytes())),
        };
...
impl RateLimiter {
    ...
    pub fn is_rate_limited(&mut self, key: &str, limit: u32) -> bool {
        ...
        let (count, last_reset) = self.call_counts.entry(key.to_string())
            .or_insert((0, now));
...
impl SipAuthenticator {
    ...
    pub async fn authenticate_request(
        ...
        let (method, request_uri) = match message {
            rsip::SipMessage::Request(req) => {
                (req.method.clone(), req.uri.clone())
            },
        ...
        if let Some(auth_header) = self.extract_authorization_header(message)? {
            return self.try_digest_authentication(&auth_header, &method, &request_uri).await;
        }
        ...
        self.record_auth_failure(source_ip, method, "Unauthorized IP address").await;
    ...
    async fn try_ip_authentication(
        ...
        let mut matching_configs: Vec<_> = self.ip_auth_configs.values()
        ...
        for config in matching_configs {
            ...
            if !config.validate_tech_prefix(tech_prefix.as_deref()) {
                debug!("Tech prefix validation failed for trunk {}: expected {:?}, got {:?}",
                    config.trunk_id, config.required_tech_prefix, tech_prefix);
                
                // Record tech prefix failure
                let reason = format!("Invalid tech prefix: expected {:?}, got {:?}", 
                    config.required_tech_prefix, tech_prefix);
                self.record_sip_failure(source_ip, FailureType::SipInvite, None, "INVITE", &reason).await;
                continue;
            }
            ...
            if let Some(rate_limit) = config.rate_limit {
                ...
                if self.rate_limiter.is_rate_limited(&rate_key, rate_limit) {
                    warn!("Rate limit exceeded for trunk {} from IP {}", config.trunk_id, source_ip);
            ...
            // Authentication successful
            info!("IP authentication successful: trunk={}, customer={}, source={}", 
    ...
    pub fn create_challenge_response(&self, request: &rsip::Request, algorithm: DigestAlgorithm) -> Result<rsip::Response> {
        ...
        response.headers.push(rsip::Header::CallId(
            request.headers.iter()
                .find_map(|h| if let rsip::Header::CallId(call_id) = h { Some(call_id.clone()) } else { None })
                .ok_or_else(|| anyhow!("No Call-ID header in request"))?
        ));
        
        response.headers.push(rsip::Header::CSeq(
            request.headers.iter()
                .find_map(|h| if let rsip::Header::CSeq(cseq) = h { Some(cseq.clone()) } else { None })
    ...
    async fn record_sip_failure(&self, ip: IpAddr, failure_type: FailureType, user: Option<&str>, method: &str, reason: &str) {
        if let Some(fail2ban) = &self.fail2ban_service {
            if let Err(e) = fail2ban.record_sip_failure(
                ip,
                failure_type,
    ...
    async fn record_auth_failure(&self, ip: IpAddr, method: rsip::Method, reason: &str) {
        ...
        self.record_sip_failure(ip, failure_type, None, &method.to_string(), reason).await;
    }
}

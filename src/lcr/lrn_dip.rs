...
use crate::lcr::types::{
    LrnCacheEntry, LrnDipConfig, LrnDipRequest, LrnDipResponse, LrnDipServer, SipRedirectResponse,
};
...
impl LrnDipService {
    ...
    pub async fn dip_lrn(&self, tn: &str, ani: Option<&str>) -> Result<LrnDipResponse> {
        ...
        if let Some(cached_entry) = self.get_cached_lrn(tn, ani) {
            if cached_entry.expires_at > Utc::now() {
                debug!(
                    "LRN cache hit for ANI/DNIS {}:{} -> {}",
                    ani.unwrap_or("anonymous"),
                    tn,
                    cached_entry.lrn
                );
                return Ok(LrnDipResponse {
    ...
    async fn perform_sip_dip(
        ...
        if let Some(primary_server) = start_server {
            ...
            match self
                .try_server(&primary_server, tn, ani, request_id, socket, &local_addr)
                .await
            {
                Ok(result) => return Ok(result),
                Err(e) => {
                    warn!(
                        "Primary LRN server {}:{} failed: {}",
        ...
        if self.config.load_balancing == "priority" {
            for server in servers.iter().skip(1) {
                debug!(
                    "Trying backup LRN server {}:{}",
                    server.server_ip, server.server_port
                );
                match self
                    .try_server(server, tn, ani, request_id, socket, &local_addr)
                    .await
                {
                    Ok(result) => return Ok(result),
                    Err(e) => {
                        warn!(
                            "Backup LRN server {}:{} failed: {}",
    ...
    async fn try_server(
        ...
        let sip_message =
            self.create_lrn_options_request(tn, ani, request_id, local_addr, &server_addr)?;
        ...
        let response_timeout = Duration::from_millis(if server.priority == 0 {
        ...
        let response = timeout(
            response_timeout,
            self.receive_sip_response(socket, request_id),
        )
        .await??;
    ...
    fn create_lrn_options_request(
        ...
        let normalized_ani = ani
            .map(|a| self.normalize_tn(a))
    ...
    pub fn extract_contact_uri(&self, response: &str) -> Result<String> {
        for line in response.lines() {
            if line.to_lowercase().starts_with("contact:") {
                ...
                return Ok(contact_line
                    .split(';')
                    .next()
                    .unwrap_or(contact_line)
                    .trim()
                    .to_string());
            }
        }
    ...
    pub fn cache_lrn_result(
        ...
        debug!(
            "Cached LRN result for ANI/DNIS {}:{} -> {}",
            ani.unwrap_or("anonymous"),
            tn,
            lrn
        );

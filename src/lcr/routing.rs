...
///
/// Given an ANI/DNIS pair and an ingress trunk, this evaluates all active
/// egress trunks, resolves the applicable vendor/client rates at the effective
/// time, applies jurisdiction and profit-protection rules, and returns the
/// candidate routes ordered by priority then cost. All rate data is served from
/// the in-memory [`LcrCache`] with a database fallback for cache misses.
pub struct RoutingEngine {
    ...
    static_route_patterns: Arc<std::sync::RwLock<HashMap<i32, Regex>>>,
}
...
impl RoutingEngine {
    pub fn new(
        ...
        Self {
            ...
            static_route_patterns: Arc::new(std::sync::RwLock::new(HashMap::new())),
        }
    }
    ...
    pub async fn find_routes(&self, request: &RouteRequest) -> Result<RouteResponse> {
        ...
        if let Some(static_route) = self.check_static_routes(
            ...
            return self.build_static_route_response(static_route, request).await;
        }
        ...
        for egress_trunk in self.cache.get_all_egress_trunks() {
            ...
            match route_type {
                RouteType::NANPA => {
                    ...
                    if let Some((rate, rate_type)) = vendor_rate_info {
                        ...
                        let (jurisdiction, lrn) = if rate_type == RateType::LRN {
                            ...
                            // Local calls are rated off local_rate, so detect them
                            // before falling back to inter/intra classification.
                            let jurisdiction = if JurisdictionCalculator::determine_local_jurisdiction(
                                &request.ani,
                                &request.dnis,
                                ani_info.as_ref(),
                                dnis_info.as_ref(),
                                ...
                                JurisdictionCalculator::determine_jurisdiction(
                                    ani_info.as_ref(),
                                    dnis_info.as_ref(),
                                )
                            };
                        ...
                        routes.push(CallRoute {
                            ...
                            vendor: format!("Vendor {}", egress_trunk.vendor_id),
                ...
                RouteType::AZ => {
                        ...
                        .await?
                    {
                            ...
                            .await?
                        {
                            ...
                            // International calls have indeterminate NANPA jurisdiction.
                ...
                RouteType::OTHER => {
                    // OTHER should have been converted to a specific type above.
                    continue;
                }
            }
        }
        ...
        let routing_decision = if total_routes > 0 {
        ...
        Ok(RouteResponse {
            ...
            lrn: overall_lrn,
            total_routes,
            ani: request.ani.clone(),
            dnis: request.dnis.clone(),
            ingress_trunk: ingress_trunk.name.clone(),
            routing_decision,
        })
    }

    /// Get client rate at a specific time, accounting for deck versioning.
    async fn get_client_rate_at_time(
        ...
        // First check if the deck is active at the given time.
    ...
    /// Get the vendor rate (and rate type) for a trunk at a specific time.
    ///
    /// `lcr_route_trunks` pins a trunk to a specific deck version, but decks are
    /// versioned over time (a new version supersedes its parent and the parent
    /// gets an `end_date`). To make deck versioning actually affect routing we
    /// resolve the deck *family* (same name + vendor) that the trunk is linked
    /// to, then pick the version active at `effective_time`.
    async fn get_vendor_rate_for_trunk_at_time(
        ...
        let deck_row = sqlx::query(
            r#"
            SELECT active_vrd.id, active_vrd.rate_type
            FROM lcr_route_trunks lrt
            JOIN vendor_rate_decks linked_vrd ON linked_vrd.id = lrt.vendor_deck_id
            JOIN vendor_rate_decks active_vrd
              ON active_vrd.name = linked_vrd.name
             AND active_vrd.vendor_id = linked_vrd.vendor_id
            WHERE lrt.egress_trunk_id = $1
              AND active_vrd.effective_date <= $2
              AND (active_vrd.end_date IS NULL OR active_vrd.end_date > $2)
              AND active_vrd.active = true
            ORDER BY active_vrd.deck_version DESC
            LIMIT 1
            "#,
        )
        .bind(trunk_id)
    ...
    /// Get the client rate for a trunk at a specific time.
    ...
    /// Load a vendor NANPA rate from the database, with prefix fallback.
    async fn load_vendor_rate_from_db(
        &self,
        deck_id: i32,
        code: &str,
    ) -> Result<Option<NanpaRate>> {
        if let Some(rate) = self.load_nanpa_rate_row("vendor_nanpa_rates", deck_id, code).await? {
            return Ok(Some(rate));
        }
        ...
        for i in (3..code.len()).rev() {
            ...
            if let Some(rate) = self
                .load_nanpa_rate_row("vendor_nanpa_rates", deck_id, prefix)
                .await?
            {
                return Ok(Some(rate));
            }
    ...
    /// Load a client NANPA rate from the database, with prefix fallback.
    async fn load_client_rate_from_db(
        &self,
        deck_id: i32,
        code: &str,
    ) -> Result<Option<NanpaRate>> {
        if let Some(rate) = self.load_nanpa_rate_row("client_nanpa_rates", deck_id, code).await? {
            return Ok(Some(rate));
        }
        ...
        for i in (3..code.len()).rev() {
            let prefix = &code[..i];
            if let Some(rate) = self
                .load_nanpa_rate_row("client_nanpa_rates", deck_id, prefix)
                .await?
            {
                return Ok(Some(rate));
            }
    ...
    /// Load a single NANPA rate row from the given table for an exact code.
    async fn load_nanpa_rate_row(
        &self,
        table: &str,
        deck_id: i32,
        code: &str,
    ...
    /// Check static routes matching the DNIS at the given position.
    fn check_static_routes(
        ...
        self.cache
            ...
            .find(|r| self.static_route_matches(r, dnis))
            .cloned()
    }

    /// Test whether a static route's pattern matches the DNIS, caching the
    /// compiled regex for reuse.
    fn static_route_matches(&self, route: &StaticRoute, dnis: &str) -> bool {
        // Fast path: pattern already compiled.
        if let Ok(patterns) = self.static_route_patterns.read() {
            if let Some(regex) = patterns.get(&route.id) {
                return regex.is_match(dnis);
            }
        }
        ...
        match Regex::new(&route.pattern) {
            Ok(regex) => {
                let matched = regex.is_match(dnis);
                if let Ok(mut patterns) = self.static_route_patterns.write() {
                    patterns.insert(route.id, regex);
                }
                matched
            }
            Err(_) => false,
    ...
    /// Build a response for a matched static route.
    async fn build_static_route_response(
        ...
        let ingress_trunk_name = self
            .cache
            .get_ingress_trunk(request.ingress_trunk_id)
            .map(|t| t.name)
            .unwrap_or_else(|| request.ingress_trunk_id.to_string());
        ...
        Ok(RouteResponse {
            ...
            ani: request.ani.clone(),
            dnis: request.dnis.clone(),
            ingress_trunk: ingress_trunk_name,
            routing_decision: "STATIC_ROUTE".to_string(),
        })
    }
    ...
    /// Load an international vendor rate with longest-to-shortest prefix matching.
    async fn load_vendor_international_rate_from_db(
        &self,
        deck_id: i32,
        dnis: &str,
    ) -> Result<Option<InternationalRate>> {
        self.load_international_rate_row("vendor_international_rates", deck_id, dnis)
            .await
    }

    /// Load an international client rate with longest-to-shortest prefix matching.
    async fn load_client_international_rate_from_db(
        &self,
        deck_id: i32,
        dnis: &str,
    ) -> Result<Option<InternationalRate>> {
        self.load_international_rate_row("client_international_rates", deck_id, dnis)
            .await
    }

    /// Load an international rate row from the given table.
    async fn load_international_rate_row(
        &self,
        table: &str,
        deck_id: i32,
        dnis: &str,
    ...
    /// Normalize an international number by removing common access prefixes.
    ...
    /// Determine whether a number is international (not NANPA).
    fn is_international_number(&self, dnis: &str) -> bool {
        let normalized = dnis.chars().filter(|c| c.is_ascii_digit()).collect::<String>();
        ...
        // - 10 digits: NPANXXNNNN
        // - 11 digits starting with 1: 1NPANXXNNNN
    ...
    /// Check if a trunk supports NANPA routing.
    fn trunk_supports_nanpa(&self, _trunk: &EgressTrunk) -> bool {
        // All trunks currently support NANPA unless configured international-only.
        true
    }

    /// Get the vendor deck ID associated with a trunk at a specific time.
    ...
    /// Get the client deck ID associated with a trunk at a specific time.
    ...
    /// Get an international routing plan configuration.

...
impl IntegratedService {
    ...
    pub fn parse_sip(
        ...
        from: SocketAddr,
        to: SocketAddr,
    ) -> Result<sip::SipMessage> {
        self.sip_parser
            .parse_message(message, from, to, sip::parser::SipTransport::UDP)
    }
    ...
    pub async fn transcode_frame(
        ...
        self.codec_service
            .transcode_frame(session_id, audio_frame)
            .await
    }
    ...
    pub fn generate_isup_iam(
        ...
        self.sipt_sipi_service
            .sip_to_iam(calling_number, called_number, cic)
    }

    /// Create SIP-T multipart body with ISUP
    pub fn create_sipt_body(&self, isup_data: &[u8], sdp: Option<&str>) -> Result<String> {

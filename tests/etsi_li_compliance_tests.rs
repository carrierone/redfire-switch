...
async fn test_warrant_expiry_enforcement() -> Result<()> {
    ...
    let active_warrants = controller
        .should_intercept(&short_warrant.target_identifier)
        .await?;
    ...
    let active_warrants = controller
        .should_intercept(&expired_warrant.target_identifier)
        .await?;

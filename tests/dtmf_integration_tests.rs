...
use redfire_switch::rfc2833_events::{
    Rfc2833Processor, Rfc2833Event, Rfc2833EventId, Rfc2833PayloadType,
    Rfc2833SdpNegotiator, Rfc2833SdpConfig
};
use redfire_switch::sip_info_dtmf::{
    SipInfoDtmfProcessor, SipInfoDtmfMessage, SipInfoDtmfContentType, 
    SipInfoPackageNegotiator, SipInfoResponse
};
use redfire_switch::sigtran_dtmf::{
    SigtranDtmfProcessor, SigtranDtmfMessage, SigtranDtmfMessageType,
    SigtranProtocol, GenericDigitsEncoding, SigtranDtmfConfig
};
use redfire_switch::stir_shaken_tdm::{
    StirShakenTdmProcessor, StirShakenTdmMessage, StirShakenTdmConfig,
    StirShakenTransport, AttestationLevel, VerificationStatus
};
...
impl DtmfIntegrationTestSuite {
    ...
    async fn new() -> Result<Self> {
        ...
        let sigtran_processor = SigtranDtmfProcessor::new(dtmf_event_sender.clone(), sigtran_config);
        
        // Create STIR/SHAKEN processor
        let stir_shaken_config = StirShakenTdmConfig::default();
        let (stir_shaken_processor, _stir_shaken_receiver) = StirShakenTdmProcessor::new(stir_shaken_config).await?;
...
async fn test_core_dtmf_processor_functionality() -> Result<()> {
    ...
    for digit in test_digits.chars() {
        let samples = generator.generate_digit(digit, Some(Duration::from_millis(100)), Some(0.5))?;
        assert!(!samples.is_empty(), "Generated samples should not be empty for digit '{}'", digit);
        assert!(samples.len() >= 800, "Should generate at least 800 samples (100ms at 8kHz) for digit '{}'", digit); // 100ms at 8kHz
    }
    
    // Test sequence generation
    let sequence_samples = generator.generate_sequence("12345", None, None, None)?;
    assert!(sequence_samples.len() > 4000, "Sequence samples should be substantial"); // Multiple digits + silence
...
async fn test_rfc2833_event_processing() -> Result<()> {
    ...
    suite.rfc2833_processor.add_payload_type(101, Rfc2833PayloadType::TelephoneEvent(101));
    ...
    for &digit in &test_digits {
        ...
        suite.rfc2833_processor.process_incoming_packet(&session_id, 101, 1000, &start_bytes).await?;
        
        let end_bytes = end_event.to_bytes()?;
        suite.rfc2833_processor.process_incoming_packet(&session_id, 101, 2000, &end_bytes).await?;
        
        // Verify DTMF event was generated
        let received_event = timeout(Duration::from_millis(100), suite.event_receiver.recv()).await
            .map_err(|_| anyhow::anyhow!("Timeout waiting for event"))?
            .ok_or_else(|| anyhow::anyhow!("Channel closed unexpectedly"))?;
        match received_event {
            DtmfEvent::DigitDetected { digit: detected_digit, source, .. } => {
                assert_eq!(detected_digit, digit);
                assert_eq!(source, DtmfSource::Rfc2833);
            }
            _ => panic!("Expected DigitDetected event for digit '{}'", digit),
        }
    }
    
    // Test packet generation
    let packets = suite.rfc2833_processor.generate_outgoing_packets(
        "test_session", '7', 200, 20, 5000
    ).await?;
    assert!(!packets.is_empty(), "Should generate RFC2833 packets");
    assert!(packets.len() >= 3, "Should include end-of-event repetitions");
...
async fn test_sip_info_dtmf_processing() -> Result<()> {
    ...
    let content_types = [
        ("application/dtmf-relay", SipInfoDtmfContentType::CiscoDtmfRelay),
        ("application/dtmf", SipInfoDtmfContentType::GenericDtmf),
        ("application/vnd.nortel.text", SipInfoDtmfContentType::NortelText),
    ];
    ...
    for (mime_type, content_type) in &content_types {
        ...
        let response = suite.sip_info_processor.process_incoming_info(
            "sip_session_123",
            "call_id_456",
            "from_tag",
            "to_tag",
        ...
        match received_event {
            ...
            _ => panic!("Expected DigitDetected event for SIP INFO"),
        }
    }
    
    // Test outgoing message generation
    let outgoing_request = suite.sip_info_processor.generate_outgoing_info(
        "test_session", '3', Some(120), Some(60)
    ).await?;
...
async fn test_sigtran_dtmf_processing() -> Result<()> {
    ...
    suite.sigtran_processor.process_incoming_message(isup_message).await?;
    
    // Should receive events for each digit in sequence
    let expected_digits = ['4', '5', '6', '7', '8', '9'];
    for &expected_digit in &expected_digits {
        let received_event = timeout(Duration::from_millis(200), suite.event_receiver.recv()).await
            .map_err(|_| anyhow::anyhow!("Timeout waiting for event"))?
            .ok_or_else(|| anyhow::anyhow!("Channel closed unexpectedly"))?;
        match received_event {
            ...
            _ => panic!("Expected DigitDetected event for digit '{}'", expected_digit),
        }
    }
    
    // Should also receive sequence complete event
    let sequence_event = timeout(Duration::from_millis(100), suite.event_receiver.recv()).await
        .map_err(|_| anyhow::anyhow!("Timeout waiting for event"))?
        .ok_or_else(|| anyhow::anyhow!("Channel closed unexpectedly"))?;
    ...
    let transaction_id = suite.sigtran_processor.start_digit_collection(
...
async fn test_stir_shaken_tdm_processing() -> Result<()> {
    ...
    let uui_data = suite.stir_shaken_processor.encode_for_isup_uui(&test_message)?;
    ...
    let decoded_message = suite.stir_shaken_processor.decode_from_isup_uui(&uui_data)?;
    assert_eq!(decoded_message.attestation_level, AttestationLevel::FullAttestation);
...
async fn test_cross_protocol_compatibility() -> Result<()> {
    ...
    suite.rfc2833_processor.process_incoming_packet(test_session, 101, 1000, &rfc2833_bytes).await?;
    
    let end_event = Rfc2833Event::end_event(Rfc2833EventId::Dtmf7, 15, 1600);
    let end_bytes = end_event.to_bytes()?;
    suite.rfc2833_processor.process_incoming_packet(test_session, 101, 2000, &end_bytes).await?;
    ...
    suite.sip_info_processor.process_incoming_info(
        test_session,
        "cross_test_call",
        "from_tag",
        "to_tag",
    ...
    suite.sigtran_processor.process_incoming_message(sigtran_message).await?;
    ...
    while event_count < 3 {
        match timeout(Duration::from_millis(500), suite.event_receiver.recv()).await {
            ...
            Err(_) => {
                panic!("Timeout waiting for DTMF event {}", event_count + 1);
            }
        }
    }
    
    // Verify all three sources were seen
    assert!(sources_seen.contains(&DtmfSource::Rfc2833), "RFC2833 source not seen");
    assert!(sources_seen.contains(&DtmfSource::SipInfo), "SIP INFO source not seen");
    assert!(sources_seen.contains(&DtmfSource::Sigtran), "Sigtran source not seen");
...
async fn test_performance_and_reliability() -> Result<()> {
    ...
    for i in 0..iterations {
        for digit in test_sequence.chars() {
            ...
            if let Some(event_id) = Rfc2833EventId::from_dtmf_char(digit) {
                ...
                suite.rfc2833_processor.process_incoming_packet(&session_id, 101, 1000, &bytes).await?;
                
                let end_event = Rfc2833Event::end_event(event_id, 10, 1200);
                let end_bytes = end_event.to_bytes()?;
                suite.rfc2833_processor.process_incoming_packet(&session_id, 101, 1500, &end_bytes).await?;
            }
        }
    }
    ...
    println!("Performance: Processed {} DTMF events in {:?} ({:.2} events/sec)", 
             events_processed, processing_time, events_per_second);
    
    // Should be able to process at least 1000 events per second
    assert!(events_per_second > 1000.0, "Performance below threshold: {:.2} events/sec", events_per_second);
    ...
    assert!(received_count >= events_processed, 
            "Event loss detected: {} received vs {} expected", received_count, events_processed);
...
async fn test_sdp_negotiation() -> Result<()> {
    ...
    let rtpmap_found = attributes.iter().any(|attr| {
        attr.contains("rtpmap") && attr.contains("telephone-event")
    });
    assert!(rtpmap_found, "Should contain rtpmap for telephone-event");
    
    let fmtp_found = attributes.iter().any(|attr| {
        attr.contains("fmtp") && attr.contains("0-15")
    });
...
async fn test_error_handling_and_edge_cases() -> Result<()> {
    ...
    let result = suite.rfc2833_processor.process_incoming_packet("test", 101, 1000, &invalid_packet).await;
    ...
    let result = suite.sip_info_processor.process_incoming_info(
        "test", "call", "from", "to", "application/dtmf-relay", invalid_sip_body
    ).await;
    assert!(result.is_err(), "Should reject invalid SIP INFO body");
    
    // Test unsupported content type
    let result = suite.sip_info_processor.process_incoming_info(
        "test", "call", "from", "to", "application/unknown", "Signal=5"
    ).await?;
    ...
    let result = suite.sigtran_processor.process_incoming_message(empty_sigtran).await;
    assert!(result.is_ok(), "Should handle empty digit sequence gracefully");
...
async fn test_realistic_telephony_scenarios() -> Result<()> {
    ...
    for (i, digit) in account_number[0..5].chars().enumerate() {
        if let Some(event_id) = Rfc2833EventId::from_dtmf_char(digit) {
            ...
            let bytes = event.to_bytes()?;
            suite.rfc2833_processor.process_incoming_packet("ivr_session", 101, 1000 + i as u32 * 1000, &bytes).await?;
            
            let end_event = Rfc2833Event::end_event(event_id, 15, 1200);
            let end_bytes = end_event.to_bytes()?;
            suite.rfc2833_processor.process_incoming_packet("ivr_session", 101, 1500 + i as u32 * 1000, &end_bytes).await?;
        }
    }
    ...
    while event_count < expected_events {
        match timeout(Duration::from_millis(1000), suite.event_receiver.recv()).await {
            ...
            Ok(None) => break,
            Err(_) => {
                println!("Timeout after receiving {} of {} digits: '{}'", 
    ...
    assert_eq!(received_digits, account_number, "Should reconstruct complete account number");

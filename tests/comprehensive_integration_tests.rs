...
async fn test_config_validation_comprehensive() -> Result<()> {
    ...
    tls_invalid.sip_profiles[1].tls_config.as_mut().unwrap().min_tls_version = "2.0".to_string();
...
async fn test_ai_analytics_integration() -> Result<()> {
    ...
    let quality_score = analytics.analyze_call_quality(
        "test-call-id",
        8000, // sample_rate
        0.02, // jitter
        10.0, // latency_ms
    ...
    let fraud_risk = analytics.detect_fraud_patterns(
        "test-caller",
...
async fn test_security_monitoring_integration() -> Result<()> {
    ...
    for i in 0..5 {
        let allowed = security_monitor.check_rate_limit(source_ip, "INVITE").await?;
        assert!(allowed, "Request {} should be allowed", i);
    }

    // Test threat detection
    let threat_detected = security_monitor.analyze_traffic_pattern(
        source_ip,
    ...
    security_monitor.add_to_blacklist(source_ip, "Automated testing", Duration::from_secs(60)).await?;
...
async fn test_codec_transcoding_integration() -> Result<()> {
    ...
    let _transcoded = codec_service.transcode_frame(
        session_id,
        &test_audio
    ).await?;
...
async fn test_system_integration_under_load() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let _quality = analytics_clone.analyze_call_quality(
                &format!("load-test-call-{}", i),
                8000,
                0.01,
                15.0,
                0.001
...
async fn test_performance_benchmarks() -> Result<()> {
    ...
    assert!(messages_per_second >= 100.0, 
           "Performance below threshold: {:.2} msg/sec", messages_per_second);

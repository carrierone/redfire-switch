...
use redfire_switch::j_std_025::{CdrEngineConfig, CdrType, CallResult};
use redfire_switch::etsi_li::{
    LiControllerConfig, DeliveryEndpoints, EncryptionAlgorithm, 
    AuthenticationMethod, DeliveryFormat
};
...
async fn test_high_throughput_cdr_generation() -> Result<()> {
    ...
    println!("Target: {} CPS for {} seconds", test_config.calls_per_second, test_config.test_duration_seconds);
    ...
    while start_time.elapsed().as_secs() < test_config.test_duration_seconds {
        ...
        join_set.spawn(async move {
            ...
            // Call attempt
            let event1 = create_perf_call_event(current_call_id, CallEventType::CallAttempt, calling_base, called_base);
            ...
            // Call answered
            let event2 = create_perf_call_event(current_call_id, CallEventType::CallAnswered, calling_base, called_base);
            let process_start = Instant::now();
            ...
            // Call ended  
            let event3 = create_perf_call_event(current_call_id, CallEventType::CallEnded, calling_base, called_base);
            let process_start = Instant::now();
    ...
    println!("Average processing time: {:.2} μs", summary.average_processing_time_us);
    println!("Error rate: {:.2}%", summary.error_rate);
    println!("Framework CDRs generated: {}", framework_stats.cdrs_generated);
    
    // Performance assertions
    assert!(summary.calls_per_second > 100.0, "Call processing rate too low");
    assert!(summary.error_rate < 1.0, "Error rate too high: {:.2}%", summary.error_rate);
    assert!(summary.average_processing_time_us < 1000.0, "Processing time too high");
...
async fn test_concurrent_call_processing() -> Result<()> {
    ...
    println!("Starting concurrent call processing test with {} calls...", concurrent_calls);
    ...
    for call_id in 0..concurrent_calls {
        ...
        join_set.spawn(async move {
            ...
            for (event_type, response_code) in events {
                let mut event = create_perf_call_event(call_id as u64, event_type, calling_base, called_base);
    ...
    println!("Processing time: {:.2} seconds", summary.test_duration.as_secs_f64());
    println!("Events per second: {:.2}", summary.events_per_second);
    println!("Average event processing time: {:.2} μs", summary.average_processing_time_us);
    println!("Framework CDRs generated: {}", framework_stats.cdrs_generated);
...
async fn test_memory_usage_under_load() -> Result<()> {
    ...
    for call_id in 0..long_running_calls {
        ...
        let event = create_perf_call_event(call_id as u64, CallEventType::CallAttempt, calling_base, called_base);
        framework.submit_call_event(event)?;
        
        let answered_event = create_perf_call_event(call_id as u64, CallEventType::CallAnswered, calling_base, called_base);
    ...
    for call_id in 0..long_running_calls {
        ...
        let event = create_perf_call_event(call_id as u64, CallEventType::CallEnded, calling_base, called_base);
        framework.submit_call_event(event)?;
    }
    ...
    assert_eq!(final_active_calls, 0, "Memory leak: calls not properly cleaned up");
...
async fn test_stress_with_li_enabled() -> Result<()> {
    ...
    for call_id in 0..stress_calls {
        ...
        join_set.spawn(async move {
            ...
            for event_type in events {
                let event = create_perf_call_event(call_id as u64, event_type, calling_base, called_base);
    ...
    assert!(summary.average_processing_time_us < 2000.0, "Processing too slow with LI");
...
async fn test_error_recovery_resilience() -> Result<()> {
    ...
    for i in 0..total_events {
        ...
        let mut event = create_perf_call_event(call_id as u64, CallEventType::CallAttempt, calling_base, called_base);
        ...
        if !inject_error {
            let end_event = create_perf_call_event(call_id as u64, CallEventType::CallEnded, calling_base, called_base);
    ...
    println!("Error rate: {:.2}%", summary.error_rate);
    println!("Framework compliance errors: {}", framework_stats.compliance_errors);
    
    // System should remain stable despite errors
    assert!(framework_stats.compliance_errors < (total_events as u64) / 2, "Too many compliance errors");
    assert!(summary.events_per_second > 0.0, "System became unresponsive");
...
async fn test_long_running_stability() -> Result<()> {
    ...
    println!("Test duration: {:.1} seconds", start_time.elapsed().as_secs_f64());
    ...
    // Stability checks
    assert_eq!(active_calls, 0, "Calls not properly cleaned up after long run");
    assert_eq!(final_stats.cdrs_generated, total_calls, "CDR count mismatch");
    assert!(final_stats.compliance_errors < total_calls / 10, "Too many errors during long run");

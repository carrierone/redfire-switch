...
mod rfc3261_tests {
    ...
    fn test_mandatory_headers_validation() {
        ...
        assert!(Rfc3261Validator::validate_message(
            &headers,
    ...
    fn test_sip_version_validation() {
        ...
        assert!(
            Rfc3261Validator::validate_message(&headers, "INVITE sip:bob@example.com SIP/1.0")
                .is_err()
        );
...
mod ani_ii_integration_tests {
    ...
    fn test_toll_free_detection() {
        ...
        for number in toll_free_numbers {
            assert!(
                RfcCompliantAniIIParser::is_toll_free(number),
                "Failed to detect {} as toll-free",
                number
            );
        }
        ...
        for number in regular_numbers {
            assert!(
                !RfcCompliantAniIIParser::is_toll_free(number),
                "Incorrectly detected {} as toll-free",
                number
            );
        }
    }
    ...
    fn test_backward_compatibility() {
        ...
        let ani_ii =
            redfire_switch::ani_ii_rfc_compliant::legacy_compatibility::parse_ani_ii_from_headers(
                &headers,
            )
            .unwrap();
        assert_eq!(ani_ii.raw_digit, 70);
...
mod error_handling_tests {
    ...
    fn test_malformed_uri_handling() {
        ...
        for uri in bad_uris {
            assert!(
                SipUriParser::parse(uri).is_err(),
                "Should reject malformed URI: {}",
                uri
            );
        }
    }
    ...
    fn test_invalid_header_field_handling() {
        ...
        for header in bad_headers {
            assert!(
                SipUriParser::parse_header_field(header).is_err(),

...
use redfire_switch::ai_analytics_engine::{AIAnalyticsEngine, AIAnalyticsConfig};
...
"#;
    ...
    let result = b2bua.process_message(invite_no_auth.as_bytes(), source).await;
    
    // Should be rejected or require authentication
    assert!(result.is_ok(), "Message processing should handle unauthenticated requests gracefully");
...
"#;
    ...
    let result = b2bua.process_message(invite_bad_auth.as_bytes(), source).await;
...
async fn test_rate_limiting_protection() {
    ...
    println!("Rate limiting test: {} messages blocked out of 100 in {:?}", blocked_count, elapsed);
    
    // Should have blocked some requests to prevent DoS
    assert!(blocked_count > 0, "Rate limiting should block some requests under rapid fire");
    assert!(blocked_count < 100, "Rate limiting shouldn't block all legitimate traffic");
}
...
async fn test_malicious_message_handling() {
    ...
    let result = b2bua.process_message(oversized_message.as_bytes(), source).await;
    assert!(result.is_ok(), "Should handle oversized messages gracefully");
...
"#;
    ...
    let result = b2bua.process_message(malformed_message.as_bytes(), source).await;
    assert!(result.is_ok(), "Should handle malformed messages gracefully");
...
"#;
    ...
    let result = b2bua.process_message(sql_injection_attempt.as_bytes(), source).await;
    assert!(result.is_ok(), "Should sanitize and handle injection attempts safely");
}
...
async fn test_buffer_overflow_prevention() {
    ...
    let test_cases = vec![
        // Very long Call-ID
        format!("INVITE sip:test@example.com SIP/2.0\r\nCall-ID: {}\r\n\r\n", "A".repeat(100000)),
    ...
    for (i, test_message) in test_cases.iter().enumerate() {
        ...
        assert!(result.is_ok(), "Buffer overflow test {} should be handled safely", i + 1);
    }
}
...
async fn test_security_event_logging() {
    ...
    for (event_type, description) in events {
        let result = security_monitor.log_security_event(
        ...
        assert!(result.is_ok(), "Security event logging should succeed for {}", event_type);
    }
    
    // Verify events can be retrieved
    let recent_events = security_monitor.get_recent_security_events(Duration::from_secs(60)).await;
    assert!(recent_events.is_ok(), "Should be able to retrieve recent security events");
    
    let events = recent_events.unwrap();
    assert!(events.len() >= 4, "Should have logged at least 4 security events");
}
...
async fn test_ip_blacklisting_functionality() {
    ...
    assert!(initial_check.is_ok() && !initial_check.unwrap(), "IP should initially be allowed");
    
    // Block the IP
    let block_result = security_monitor.block_ip(
        malicious_ip,
    ...
    assert!(blocked_check.is_ok() && blocked_check.unwrap(), "IP should be blocked after blocking");
    
    // Test that blocked IP cannot make requests
    let rate_limit_check = security_monitor.check_rate_limit(malicious_ip, "INVITE").await;
    assert!(rate_limit_check.is_ok() && !rate_limit_check.unwrap(), "Blocked IP should be rate limited");
}
...
async fn test_fraud_detection_integration() {
    ...
    let multiple_destinations = vec![
        "19991234567", "19991234568", "19991234569", 
        "19991234570", "19991234571", "19991234572"
    ];
    
    for destination in &multiple_destinations {
        let fraud_result = analytics.detect_fraud_patterns(
            suspicious_caller,
            &[destination],
            1,
        ...
        assert!(fraud_result.is_ok(), "Fraud detection should process calls successfully");
    }
    
    // Test detection of suspicious calling patterns
    let pattern_result = analytics.detect_fraud_patterns(
        suspicious_caller,
    ...
    assert!(pattern_result.is_ok(), "Should analyze suspicious calling patterns");
    
    // Verify the fraud score increases with suspicious activity
    let fraud_score = pattern_result.unwrap();
    println!("Fraud detection score for suspicious pattern: {}", fraud_score);
    assert!(fraud_score >= 0.0 && fraud_score <= 1.0, "Fraud score should be between 0 and 1");
}
...
async fn test_tls_certificate_validation() {
    ...
    if let Some(tls_config) = &config.tls_config {
        assert!(!tls_config.certificate_path.is_empty(), "Certificate path should be configured");
        assert!(!tls_config.private_key_path.is_empty(), "Private key path should be configured");
        assert!(tls_config.min_tls_version >= 1.2, "Should require TLS 1.2 or higher");
    }
...
async fn test_session_security_limits() {
    ...
    for i in 1..=100 {
        ...
        let invite_message = format!(r#"INVITE sip:test{}@example.com SIP/2.0
...
"#, i, i, i, i, i, i, i, i, i, i);
        ...
        let handle = tokio::spawn({
            ...
            async move {
                b2bua.process_message(message.as_bytes(), source).await
            }
        });
    ...
    println!("Successfully processed {} out of 100 concurrent sessions", successful_sessions);
    
    // Should handle reasonable number of concurrent sessions
    assert!(successful_sessions > 50, "Should handle reasonable concurrent load");
}
...
async fn test_input_sanitization() {
    ...
    for (i, malicious_input) in malicious_inputs.iter().enumerate() {
        let invite_with_injection = format!(r#"INVITE sip:test@example.com SIP/2.0
Via: SIP/2.0/UDP attacker.example.org:5060;branch=z9hG4bK-test-branch
...
"#, i, malicious_input, i, i, malicious_input);
        ...
        let result = b2bua.process_message(invite_with_injection.as_bytes(), source).await;
        assert!(result.is_ok(), "Should safely handle injection attempt {}: {}", i + 1, malicious_input);
    }
}
...
async fn test_security_monitoring_alerts() {
    ...
    let analysis_result = security_monitor.analyze_traffic_pattern(
        test_ip,
        suspicious_patterns,
        Duration::from_secs(5)
    ).await;
    
    assert!(analysis_result.is_ok(), "Traffic pattern analysis should complete successfully");
    
    let threat_level = analysis_result.unwrap();
    println!("Detected threat level: {}", threat_level);
    assert!(threat_level >= 0.0 && threat_level <= 1.0, "Threat level should be normalized 0-1");
}
    ...
    for attacker_id in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            for request_id in 0..50 {
                ...
                // Send attack message
                let attack_message = format!(r#"INVITE sip:victim@example.com SIP/2.0
...
"#, attacker_id, attacker_id, request_id, attacker_id, attacker_id, 
                                attacker_id, request_id, attacker_id, request_id, request_id, attacker_id);
                
                let _ = b2bua_clone.process_message(attack_message.as_bytes(), source).await;
    ...
    assert!(attack_duration < Duration::from_secs(60), "System should handle attack within reasonable time");
    
    // Verify security systems are still functional
    let health_check = security_monitor.get_recent_security_events(Duration::from_secs(300)).await;
    assert!(health_check.is_ok(), "Security monitoring should remain functional after attack");
}
...
fn bench_sip_message_throughput(c: &mut Criterion) {
    ...
    let config = rt.block_on(async {
        Config::load_from_file("config-production-example.json").unwrap_or_else(|_| Config::default())
    });
    
    let b2bua = rt.block_on(async {
        SimpleB2BUA::new(config).await.unwrap()
    });
                ...
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        for i in 0..batch_size {
                            ...
                            let _ = b2bua.process_message_async(
                        ...
                        if batch_size >= 1000 && throughput < 100_000.0 {
                            eprintln!("WARNING: Throughput {} msg/sec below target for batch size {}", 
    ...
    c.bench_function("response_latency", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let _ = b2bua.process_message(
                    black_box(test_invite.as_bytes()), 
                    black_box(source)
                ).await;
...
fn bench_concurrent_sessions(c: &mut Criterion) {
    ...
    let b2bua = Arc::new(rt.block_on(async {
        SimpleB2BUA::new(config).await.unwrap()
    }));
                    ...
                    rt.block_on(async {
                        ...
                        for i in 0..session_count {
                            ...
                            let handle = tokio::spawn(async move {
                                b2bua_clone.process_message(
                                    test_invite.as_bytes(),
                                    source
                                ).await
                            });
                        ...
                        println!("Processed {}/{} sessions at {:.2} sessions/sec in {:?}", 
                        ...
                        if session_count >= 1000 && successful < session_count * 95 / 100 {
                            eprintln!("WARNING: Only {}/{} sessions successful", successful, session_count);
                        }
                ...
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        for _ in 0..100 {
                            ...
                            let _ = codec_service.transcode_frame(
                                &session_id,
...
fn bench_security_monitoring(c: &mut Criterion) {
    ...
    let security_monitor = rt.block_on(async {
        SecurityMonitor::new().await.unwrap()
    });
    ...
    c.bench_function("security_rate_limiting", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                for _ in 0..1000 {
                    let _ = security_monitor.check_rate_limit(
                        black_box(source_ip), 
                        black_box("INVITE")
                    ).await;
                }
    ...
    c.bench_function("threat_detection", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let _ = security_monitor.analyze_traffic_pattern(
                    black_box(source_ip),
    ...
    c.bench_function("call_quality_analysis", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let _ = analytics.analyze_call_quality(
    ...
    c.bench_function("fraud_detection", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let _ = analytics.detect_fraud_patterns(
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                for i in 0..test_messages {
                    ...
                    let _ = b2bua.process_message_async(message.as_bytes(), source).await;
                }
                ...
                assert!(messages_per_second >= 1000.0, "Throughput below minimum threshold");
                assert!(single_message_latency <= Duration::from_millis(10), "Latency above threshold");

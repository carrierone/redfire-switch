...
use redfire_switch::g729_external_asm::{L_FRAME, L_WINDOW, ExternalAsmG729Codec};
...
fn bench_dsp_functions(c: &mut Criterion) {
    ...
    let autocorr_data = [100.0, 80.0, 60.0, 40.0, 30.0, 20.0, 15.0, 10.0, 5.0, 2.0, 1.0];
    
    // Benchmark autocorrelation with external assembly
    let codec = ExternalAsmG729Codec::new();
    if codec.get_performance_stats().cpu_features.avx { // AVX available
    ...
    group.bench_function("optimized_g729", |b| {
        b.iter(|| {
            ...
            if simd_ops + fallback_ops > 0 {
                eprintln!("SIMD usage: {:.1}% ({} SIMD, {} fallback)", 
...
fn print_system_info() {
    ...
    if let Ok(cpu_info) = std::fs::read_to_string("/proc/cpuinfo") {
        if let Some(model_line) = cpu_info.lines()
            .find(|line| line.starts_with("model name")) {
            println!("CPU: {}", model_line.split(':').nth(1).unwrap_or("Unknown").trim());
        }
        
        let cpu_count = cpu_info.lines()
            .filter(|line| line.starts_with("processor"))
            .count();
        println!("CPU Cores: {}", cpu_count);
    }
    
    println!("G.729 Frame Size: {} samples ({} ms)", L_FRAME, L_FRAME * 1000 / 8000);
...
mod benchmark_tests {
    ...
    fn test_performance_stats() {
        ...
        let (simd_ops, fallback_ops, ratio) = codec.get_performance_stats();
        println!("Performance stats: SIMD={}, Fallback={}, Ratio={:.1}%", 

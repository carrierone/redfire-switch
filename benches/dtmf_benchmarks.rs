...
fn bench_dtmf_generation(c: &mut Criterion) {
    ...
    for &duration_ms in &[50, 100, 200, 500] {
        group.bench_with_input(
            ...
            |b, &duration_ms| {
                b.iter(|| {
                    black_box(generator.generate_digit(
                        '5',
    ...
    for sequence in &sequences {
        group.bench_with_input(
            ...
            |b, &sequence| {
                b.iter(|| {
                    black_box(generator.generate_sequence(sequence, None, None, None).unwrap());
                })
            }
        );
    }
...
fn bench_dtmf_detection(c: &mut Criterion) {
    ...
    let test_samples = generator.generate_digit('5', Some(Duration::from_millis(200)), None).unwrap();
    
    // Benchmark audio processing
    group.bench_function("process_audio", |b| {
        b.to_async(&rt).iter(|| async {
            let detector_clone = Arc::clone(&detector);
            detector_clone.add_channel("bench_channel".to_string()).await.unwrap();
            black_box(detector_clone.process_audio("bench_channel", &test_samples, DtmfSource::Internal).await.unwrap());
            detector_clone.remove_channel("bench_channel").await.unwrap();
        })
    });
...
fn bench_rfc2833_processing(c: &mut Criterion) {
    ...
    let test_events: Vec<_> = "1234567890*#ABCD".chars()
    ...
    group.bench_function("single_event", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            black_box(processor_clone.process_incoming_packet(
                "bench_session",
                101,
                1000,
                &test_events[0]
    ...
    group.bench_function("packet_generation", |b| {
        b.to_async(&rt).iter(|| async {
            let processor_clone = Arc::clone(&processor);
            black_box(processor_clone.generate_outgoing_packets(
                "bench_session",
                '5',
                200,
                20,
...
fn bench_sip_info_processing(c: &mut Criterion) {
    ...
    let generic_message = SipInfoDtmfMessage::new('7', SipInfoDtmfContentType::GenericDtmf)
        .with_duration(150);
    ...
    group.bench_function("parse_cisco_format", |b| {
        b.iter(|| {
            black_box(SipInfoDtmfMessage::from_body_content(
    ...
    group.bench_function("process_incoming", |b| {
        b.to_async(&rt).iter(|| async {
            let processor_clone = Arc::clone(&processor);
            black_box(processor_clone.process_incoming_info(
                "bench_session",
                "bench_call",
                "from_tag",
                "to_tag",
    ...
    group.bench_function("generate_outgoing", |b| {
        b.to_async(&rt).iter(|| async {
            let processor_clone = Arc::clone(&processor);
            black_box(processor_clone.generate_outgoing_info(
                "bench_session",
                '5',
                Some(120),
                Some(60)
...
fn bench_sigtran_processing(c: &mut Criterion) {
    ...
    for (i, message) in test_messages.iter().enumerate() {
        group.bench_with_input(
            ...
            |b, message| {
                b.to_async(&rt).iter(|| async {
                    let processor_clone = Arc::clone(&processor);
                    black_box(processor_clone.process_incoming_message(message.clone()).await.unwrap());
                })
            }
        );
    }
    ...
    group.bench_function("create_isup_parameter", |b| {
        b.iter(|| {
            black_box(processor.create_isup_generic_digits(
                "12345",
    ...
    group.bench_function("bcd_encode", |b| {
        b.iter(|| {
            black_box(processor.encode_digits("1234567890*#ABC", GenericDigitsEncoding::BcdEven).unwrap());
        })
    });
    
    group.bench_function("bcd_decode", |b| {
        let encoded = processor.encode_digits("1234567890*#ABC", GenericDigitsEncoding::BcdEven).unwrap();
        let encoded_str = String::from_utf8_lossy(&encoded);
        b.iter(|| {
            black_box(processor.decode_digits(&encoded_str, GenericDigitsEncoding::BcdEven).unwrap());
        })
    });
...
fn bench_concurrent_processing(c: &mut Criterion) {
    ...
    let sigtran_processor = Arc::new(SigtranDtmfProcessor::new(event_sender, SigtranDtmfConfig::default()));
                ...
                b.to_async(&rt).iter(|| async {
                    ...
                    for i in 0..num_sessions {
                        ...
                        let handle = tokio::spawn(async move {
                            ...
                            processor_clone.process_incoming_packet(&session_id, 101, 1000, &bytes).await.unwrap();
                            
                            let end_event = Rfc2833Event::end_event(Rfc2833EventId::Dtmf5, 10, 1200);
                            let end_bytes = end_event.to_bytes().unwrap();
                            processor_clone.process_incoming_packet(&session_id, 101, 1500, &end_bytes).await.unwrap();
                        });
                        handles.push(handle);
                    }

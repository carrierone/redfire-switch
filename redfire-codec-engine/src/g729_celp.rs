...
impl G729Encoder {
        ...
        if !is_speech && self.vad_enable {
            ...
            for subframe in 0..2 {
                ...
                if subframe == 0 {
                    ...
                    self.pack_bits(&mut bitstream, &mut bit_index, 
                                  self.quantize_gains(pitch_gain, fixed_gain) as u32, 7);
                } else {
                    // Second subframe: 5+13+4+3 = 25 bits
                    self.pack_bits(&mut bitstream, &mut bit_index, 
                                  (pitch_delay - 18) as u32, 5); // Differential
                    self.pack_bits(&mut bitstream, &mut bit_index, fixed_index as u32, 13);
                    self.pack_bits(&mut bitstream, &mut bit_index,
                                  self.quantize_gains(pitch_gain, fixed_gain) as u32, 7);
                }
    ...
    /// Update excitation buffer
    fn update_excitation(&mut self, pitch_delay: usize, pitch_gain: f32, 
                         fixed_index: usize, fixed_gain: f32) {
        ...
        for i in 0..L_SUBFR {
            ...
            for pulse in 0..4 {
                ...
                if pos == i {
                    let sign = if (fixed_index >> (12 - pulse)) & 1 == 1 { -1.0 } else { 1.0 };
                    exc += fixed_gain * sign;
                }
            }
...
impl G729Decoder {
    ...
    pub fn decode(&mut self, bitstream: &[u8]) -> Result<Vec<i16>> {
        ...
        if is_sid {
            ...
            for subframe in 0..2 {
                ...
                // Generate excitation
                let exc = self.generate_excitation(pitch_delay, pitch_gain, 
                                                  fixed_index, fixed_gain);
    ...
    fn generate_excitation(&mut self, pitch_delay: usize, pitch_gain: f32,
                          fixed_index: usize, fixed_gain: f32) -> Vec<f32> {

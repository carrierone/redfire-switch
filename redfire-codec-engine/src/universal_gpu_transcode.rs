...
impl UniversalGpuTranscoder {
    ...
    fn load_kernels(&mut self) -> Result<()> {
        ...
        let universal_kernel = self
            .device
            .get_func("universal_transcode", "universal_transcode_kernel")?;
        let ulaw_alaw_kernel = self
            .device
            .get_func("universal_transcode", "g711_ulaw_alaw_direct_kernel")?;
        let alaw_ulaw_kernel = self
            .device
            .get_func("universal_transcode", "g711_alaw_ulaw_direct_kernel")?;
        let batch_kernel = self
            .device
            .get_func("universal_transcode", "batch_universal_transcode_kernel")?;

        self.kernels
            .insert("universal".to_string(), universal_kernel);
        self.kernels
            .insert("ulaw_to_alaw".to_string(), ulaw_alaw_kernel);
        self.kernels
            .insert("alaw_to_ulaw".to_string(), alaw_ulaw_kernel);
    ...
    pub async fn batch_transcode(
        ...
        let (output_sizes, output_offsets) =
            self.calculate_output_batch(input_frames, target_codecs)?;
        ...
        // Copy data to GPU
        self.device
            .dtoh_sync_copy_into(&input_data, &self.device_memory_pool.input_buffer)?;
        self.device
            .dtoh_sync_copy_into(&input_sizes, &self.device_memory_pool.input_sizes)?;
        self.device
            .dtoh_sync_copy_into(&output_sizes, &self.device_memory_pool.output_sizes)?;
        self.device
            .dtoh_sync_copy_into(&src_codecs, &self.device_memory_pool.src_codecs)?;
        self.device
            .dtoh_sync_copy_into(&dst_codecs, &self.device_memory_pool.dst_codecs)?;
        self.device
            .dtoh_sync_copy_into(&input_offsets, &self.device_memory_pool.input_offsets)?;
        self.device
            .dtoh_sync_copy_into(&output_offsets, &self.device_memory_pool.output_offsets)?;

        // Launch kernel
        let kernel = self
            .kernels
            .get("batch")
            .ok_or_else(|| anyhow!("Batch kernel not found"))?;
        ...
        self.device
            .dtoh_sync_copy_into(&self.device_memory_pool.output_buffer, &mut output_data)?;
    ...
    async fn try_direct_conversion(
        ...
        match (input.codec, target) {
            (AudioCodec::G711Ulaw, AudioCodec::G711Alaw) => {
                ...
                let result = self
                    .run_direct_kernel(kernel, &input.data, input.data.len())
                    .await?;
    ...
    async fn run_direct_kernel(
        ...
        unsafe {
            kernel.launch_async(
                config,
                (&input_gpu, &output_gpu, input_data.len() as i32),
                &self.stream,
            )?;
        }
...
impl UniversalGpuTranscoder {
    ...
    pub async fn transcode_frame(
        &self,
        _input_frame: &AudioFrame,
        _target_codec: AudioCodec,
    ) -> Result<TranscodedFrame> {
        Err(anyhow!("CUDA feature not enabled"))
    }
    ...
    pub async fn batch_transcode(
        &self,
        _input_frames: &[AudioFrame],
        _target_codecs: &[AudioCodec],
    ) -> Result<Vec<TranscodedFrame>> {
        Err(anyhow!("CUDA feature not enabled"))
    }

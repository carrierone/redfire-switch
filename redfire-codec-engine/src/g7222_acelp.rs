...
impl G7222Encoder {
    ...
    pub fn encode(&mut self, pcm_input: &[i16]) -> Result<Vec<u8>> {
        ...
        for subframe in 0..NB_SUBFR_WB {
            ...
            self.pack_subframe_params(&mut bitstream, &mut bit_index, 
                                    pitch_delay, pitch_gain, 
                                    &pulse_indices, fixed_gain, subframe);
    ...
    fn apply_high_pass_filter(&mut self, speech: &mut [f32]) {
        ...
        for i in 0..speech.len() {
            ...
            for j in 1..5 {
                if i >= j {
                    output += b[j] * (if i - j < speech.len() { speech[i - j] } else { 0.0 });
                }
    ...
    fn compute_lp_analysis_wb(&self) -> Vec<f32> {
        ...
        let lag_window = [
            1.0000000, 0.9999951, 0.9999804, 0.9999559, 0.9999216,
            0.9998774, 0.9998234, 0.9997596, 0.9996860, 0.9996025,
            0.9995092, 0.9994061, 0.9992931, 0.9991703, 0.9990376,
            0.9988951, 0.9987428
        ];
        for i in 1..=M_WB {
            r[i] *= lag_window[i];
    ...
    fn find_polynomial_root(&self, poly: &[f32], mut low: f32, mut high: f32) -> f32 {
        for _ in 0..15 { // More iterations for wideband precision
    ...
    fn adaptive_codebook_search_wb(&self, target: &[f32], subframe: usize) -> (usize, f32) {
        ...
        let (pit_min, pit_max) = if subframe == 0 {
            ...
            ((last_pitch - 8).max(PIT_MIN_WB), (last_pitch + 8).min(PIT_MAX_WB))
        };
    ...
    /// Pack subframe parameters
    fn pack_subframe_params(&self, bitstream: &mut [u8], bit_index: &mut usize,
                           pitch_delay: usize, pitch_gain: f32,
                           pulse_indices: &[usize], fixed_gain: f32,
                           subframe: usize) {
        // Pack pitch delay
        if subframe == 0 {
    ...
    fn update_excitation_wb(&mut self, pitch_delay: usize, pitch_gain: f32,
                           pulse_indices: &[usize], fixed_gain: f32) {
...
impl G7222Decoder {
    ...
    pub fn decode(&mut self, bitstream: &[u8]) -> Result<Vec<i16>> {
        ...
        for subframe in 0..NB_SUBFR_WB {
            ...
            // Generate excitation
            let exc = self.generate_excitation_wb(pitch_delay, pitch_gain, 
                                                &pulse_indices, fixed_gain);
    ...
    fn unpack_subframe_params(&self, bitstream: &[u8], bit_index: &mut usize, subframe: usize) 
    ...
    fn generate_excitation_wb(&mut self, pitch_delay: usize, pitch_gain: f32,
                             pulse_indices: &[usize], fixed_gain: f32) -> Vec<f32> {
...
mod tests {
    ...
    fn test_decoder_creation() {
        ...
        assert_eq!(decoder.old_exc.len(), L_FRAME_WB + PIT_MAX_WB + L_INTERPOL_WB);
    }

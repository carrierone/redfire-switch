...
pub use state::{
    SipStateManager, SipStateConfig, SipStateAction, TransactionTimerManager,
};

pub use authentication::{
    SipAuthenticator, AuthResult, AuthFailureReason, DigestAlgorithm,
    IpAuthConfig, DigestCredentials, RateLimiter,
};

pub use transport::{
    SipTransport, TransportConfig, TlsConfig, TransportMessage,
    ConnectionInfo, TransportEvent, SipTransportManager,
};

pub use core::{
    SipCoreConfig, SipCallContext, SipRequestResult, SipCoreEngine, ProcessorMessage,
};

pub use debug_cli::{
    SipDebugConfig, ColorScheme, ExportFormat, SipDebugFilter,
    SipDebugMessage, MessageDirection, TrunkInfo, TrunkType, CallInfo, MessageTiming,
};

pub use interop::{
    SipInteropConfig, SipStackType, StackSpecificConfig, InteropQuirk,
    SipExtension, SessionTimerConfig, PrackConfig, DialogEventConfig,
    SipSecurityConfig, SdpPreferences,
};

pub use compliance::{
    ComplianceLevel, RfcImplementation, ImplementationStatus,
    SipComplianceChecker, ComplianceReport, InteropConfigGenerator,
};

pub use sipt_sipi::{
    SipTSipIService, SipTSipIConfig, IsupMessage, IsupMessageType,
    IsupParameter, IsupParameterType, IsupVariant, ForwardCallIndicators,
...
pub mod utils {
    ...
    pub fn default_port_for_transport(transport: &crate::transport::SipTransport) -> u16 {
        match transport {
            crate::transport::SipTransport::Udp | crate::transport::SipTransport::Tcp => DEFAULT_SIP_PORT,
            crate::transport::SipTransport::Tls | crate::transport::SipTransport::Wss => DEFAULT_SIPS_PORT,
        }
    }
}
...
mod tests {
    ...
    fn test_utils() {
        ...
        assert_eq!(utils::default_port_for_transport(&crate::transport::SipTransport::Udp), 5060);
        assert_eq!(utils::default_port_for_transport(&crate::transport::SipTransport::Tls), 5061);
    }

...
use crate::authentication::{AuthResult, SipAuthenticator};
use crate::parser::{SipMessage, SipParser};
use crate::state::{SipStateAction, SipStateManager};
use crate::transport::{
    SipTransport, SipTransportManager, TransportConfig, TransportEvent, TransportMessage,
};
            ...
            transports: vec![
                TransportConfig {
                    ...
                    bind_address: "0.0.0.0:5060"
                        .parse()
                        .expect("Default UDP bind address must be valid"),
                ...
                TransportConfig {
                    ...
                    bind_address: "0.0.0.0:5060"
                        .parse()
                        .expect("Default TCP bind address must be valid"),
...
impl SipCoreEngine {
    ...
    pub async fn process_request(&self, message: TransportMessage) -> Result<SipRequestResult> {
        ...
        self.message_processor
            .send(ProcessorMessage::ProcessRequest {
                message,
    ...
    pub fn set_compliance_framework(
        &mut self,
        framework: Arc<dyn ComplianceNotifier + Send + Sync>,
    ) {
    ...
    fn notify_compliance(
        ...
        if let Some(ref framework) = self.compliance_framework {
            match event_type {
                ...
                ComplianceEventType::SipMethod {
                    method,
                    response_code,
                    source_ip,
                } => {
                    framework.notify_sip_method(
                        &context.call_id,
                        &method,
                        response_code,
                        source_ip,
    ...
    pub async fn send_request(
        ...
        self.transport_manager
            .send_message(&rsip::SipMessage::Request(request), destination, transport)
            .await
    }
}
...
impl SipMessageProcessor {
    ...
    async fn handle_transport_event(&self, event: TransportEvent) {
        match event {
            ...
            TransportEvent::ConnectionEstablished {
                ...
                debug!(
                    "New connection established: {} from {} via {:?}",
                    connection_id, remote_addr, transport
                );
            }
    ...
    async fn process_sip_request(&self, message: TransportMessage) -> SipRequestResult {
        ...
        if self.config.enable_authentication {
            let auth_result = match self
                ...
                .write()
                .await
                .authenticate_request(&sip_msg.message, message.source.ip())
                .await
            {
                Ok(result) => result,
                Err(e) => {
            ...
            match auth_result {
                AuthResult::Authorized {
                    ...
                    debug!(
                        "Request authorized: trunk={}, customer={}",
                        trunk_id, customer_id
                    );
                    return self
                        .process_authorized_request(
                            request,
                            &sip_msg,
                            message,
                            trunk_id,
    ...
    async fn process_authorized_request(
        ...
        match state_action {
            ...
            SipStateAction::ProcessReInvite {
                transaction_id: _,
                dialog_id: _,
            } => {
            ...
            SipStateAction::ProcessCancel {
                transaction_id: _,
                invite_transaction_id: _,
            } => {
            ...
            SipStateAction::ProcessOtherRequest {
                transaction_id: _,
                method: _,
            } => {
            ...
            SipStateAction::ProcessResponse {
                transaction_id: _,
                dialog_id: _,
                requires_ack: _,
            } => {

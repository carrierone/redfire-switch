...
impl SipTransportManager {
    ...
    async fn start_udp_transport(&self, config: &TransportConfig) -> Result<()> {
        let socket = UdpSocket::bind(config.bind_address).await.map_err(|e| {
            anyhow!(
    ...
    async fn start_tls_transport(&self, config: &TransportConfig) -> Result<()> {
        ...
        let listener = TcpListener::bind(config.bind_address).await.map_err(|e| {
            anyhow!(
                "Failed to bind TLS listener to {}: {}",
                config.bind_address,
                e
            )
        })?;
            ...
            match reader.read(&mut buffer).await {
                ...
                Ok(size) => {
                    ...
                    match Self::parse_sip_message(message_data) {
                        ...
                        Err(e) => {
                            debug!(
                                "Failed to parse SIP message from TCP connection {}: {}",
                                connection_id, e
                            );
                        }
                    }
            ...
            match reader.read(&mut buffer).await {
                ...
                Ok(size) => {
                    ...
                    match Self::parse_sip_message(message_data) {
                        ...
                        Err(e) => {
                            debug!(
                                "Failed to parse SIP message from TLS connection {}: {}",
                                connection_id, e
                            );
                        }
                    }
        ...
        match transport {
            ...
            SipTransport::Tls => {
                if let Some(connector) = &self.tls_connector {
                    ...
                    let domain =
                        rustls::ServerName::try_from(destination.ip().to_string().as_str())

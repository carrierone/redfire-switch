...
fn benchmark_rust_g729(test_signals: &[Vec<i16>]) -> (f64, Vec<Vec<u8>>) {
    ...
    println!(
        "Rust G.729: {} frames in {:.2} ms ({:.2} frames/sec)",
        encoded_frames.len(),
        duration_ms,
        encoded_frames.len() as f64 / duration_ms * 1000.0
    );
...
fn benchmark_optimized_g729(test_signals: &[Vec<i16>]) -> (f64, Vec<Vec<u8>>) {
    ...
    println!(
        "Optimized G.729: {} frames in {:.2} ms ({:.2} frames/sec)",
        encoded_frames.len(),
        duration_ms,
        encoded_frames.len() as f64 / duration_ms * 1000.0
    );
    ...
    println!(
        "SIMD usage: {:.1}% ({} SIMD ops, {} fallback ops)",
...
fn main() {
    ...
    println!(
        "System SIMD support - SSE: {}, AVX: {}, FMA: {}",
        sse, avx, fma
    );
    ...
    if rust_time > 0.0 {
        ...
        if rust_encoded.len() == opt_encoded.len() {
            ...
            for (i, (rust_frame, opt_frame)) in rust_encoded
                .iter()
                .zip(opt_encoded.iter())
                .enumerate()
                .take(10)
            {
            ...
            if differences == 0 {
                ...
                println!(
                    "Output consistency: {} differences in first 10 frames",
                    differences
                );
    ...
    println!(
        "Rust G.729 real-time capable: {}",
        if rust_time <= required_time {
            "Yes"
        } else {
            "No"
        }
    ...
    println!(
        "Optimized G.729 real-time capable: {}",
        if opt_time <= required_time {
            "Yes"
        } else {
            "No"
        }

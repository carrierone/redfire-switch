...
use redfire_switch::g729_external_asm::{
    ExternalAsmG729Codec, autocorrelation_optimized, levinson_durbin_optimized,
    lsp_quantization_optimized, L_FRAME, L_WINDOW
};
...
fn test_cpu_features() {
    ...
    println!("Assembly optimization available: {}", stats.assembly_available);
...
fn test_dsp_functions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  r[0] = {:.3}, r[1] = {:.3}, r[2] = {:.3}", r[0], r[1], r[2]);
    ...
    println!("  LP coeffs[0-2]: [{:.3}, {:.3}, {:.3}]", lp_coeffs[0], lp_coeffs[1], lp_coeffs[2]);
    println!("  ✓ lp_coeffs[0] == 1.0: {}", (lp_coeffs[0] - 1.0).abs() < 0.001);
    
    // Test LSP quantization
    println!("\nTesting LSP quantization...");
    let lsp = [0.314, 0.628, 0.942, 1.256, 1.570, 1.884, 2.198, 2.512, 2.826, 3.140];
    ...
    println!("  Best index: {}, Min distance: {:.6}", best_index, min_distance);
...
fn test_codec_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Real-time capable (≤10ms): {} {}", 
...
fn run_comprehensive_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &frames in &test_sizes {
        ...
        println!("  Time: {:?} ({:.3} ms/frame)", total_time, avg_time_per_frame);
    ...
    println!("  Codec state size: ~{} bytes", std::mem::size_of::<ExternalAsmG729Codec>());
    println!("  Frame size: {} samples ({} bytes)", L_FRAME, L_FRAME * 4);
    println!("  Analysis window: {} samples ({} bytes)", L_WINDOW, L_WINDOW * 4);

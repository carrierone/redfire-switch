...
use redfire_switch::dtmf_processor::{
    DtmfProcessor, DtmfDetectorConfig, DtmfGeneratorConfig, DtmfEvent, DtmfSource
};
...
impl DtmfShowcase {
    ...
    async fn demonstrate_core_dtmf(&self) -> Result<()> {
        ...
        for digit in test_sequence.chars() {
            // Generate DTMF tone
            let samples = generator.generate_digit(digit, Some(Duration::from_millis(150)), Some(0.7))?;
            println!("  🔊 Generated {} samples for digit '{}'", samples.len(), digit);
            
            // Process for detection (simulated)
            detector.process_audio("showcase_channel", &samples, DtmfSource::Internal).await?;
        ...
        println!("🎼 Generated sequence with {} total samples", sequence_samples.len());
        
        // Get detection statistics
        let stats = detector.get_statistics("showcase_channel").await?;
        println!("📊 Detection stats - Channel: {}, Active: {}, Sequence: '{}'", 
    ...
    async fn demonstrate_rfc2833(&mut self) -> Result<()> {
        ...
        for &digit in &test_digits {
            ...
            self.rfc2833_processor.process_incoming_packet(&session_id, 101, 1000, &start_bytes).await?;
            
            // End event
            let end_event = Rfc2833Event::end_event(event_id, 12, 1600);
            let end_bytes = end_event.to_bytes()?;
            self.rfc2833_processor.process_incoming_packet(&session_id, 101, 2000, &end_bytes).await?;
            
            println!("  ✅ Processed RFC2833 event for digit '{}'", digit);
        }
        
        // Generate outgoing RFC2833 packets
        let outgoing_packets = self.rfc2833_processor.generate_outgoing_packets(
            "outgoing_session", '7', 200, 15, 5000
        ).await?;
        println!("📤 Generated {} RFC2833 packets for outgoing digit '7'", outgoing_packets.len());
    ...
    async fn demonstrate_sip_info(&mut self) -> Result<()> {
        ...
        let content_types = [
            ("Cisco DTMF-Relay", "application/dtmf-relay", SipInfoDtmfContentType::CiscoDtmfRelay),
            ("Generic DTMF", "application/dtmf", SipInfoDtmfContentType::GenericDtmf),
            ("Nortel Text", "application/vnd.nortel.text", SipInfoDtmfContentType::NortelText),
        ];
        ...
        for (name, mime_type, content_type) in &content_types {
            ...
            println!("  📝 Generated body: {}", body_content.replace('\r', "\\r").replace('\n', "\\n"));
            
            // Process incoming SIP INFO
            let response = self.sip_info_processor.process_incoming_info(
                "sip_session_123",
                "call_id_456",
                "from_tag",
                "to_tag",
        ...
        let outgoing_request = self.sip_info_processor.generate_outgoing_info(
            "test_session", '3', Some(150), Some(70)
        ).await?;
        ...
        println!("  Body: {}", outgoing_request.body.replace('\r', "\\r").replace('\n', "\\n"));
        
        // Session statistics
        let session_stats = self.sip_info_processor.get_session_stats("sip_session_123").await;
        if let Some(stats) = session_stats {
            println!("📊 Session stats - ID: {}, Call: {}, Sequence: '{}'", 
    ...
    async fn demonstrate_sigtran(&mut self) -> Result<()> {
        ...
        self.sigtran_processor.process_incoming_message(isup_message).await?;
        println!("  ✅ Processed ISUP Generic Digits: '987654321'");
        
        // TCAP transaction management
        println!("🔄 Managing TCAP transactions");
        let transaction_id = self.sigtran_processor.start_digit_collection(
            SigtranProtocol::M3ua, 
            Some(200)
        ).await;
        ...
        self.sigtran_processor.process_incoming_message(tcap_message).await?;
        ...
        for stat in tx_stats {
            println!("  Transaction {}: Protocol {:?}, CIC {:?}, Digits: '{}'", 
    ...
    async fn demonstrate_stir_shaken(&self) -> Result<()> {
        ...
        let test_message = StirShakenTdmMessage {
            ...
            passport_token: "eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCJ9.test_payload.mock_signature".to_string(),
        ...
        println!("📞 STIR/SHAKEN call: {} -> {}", calling_number, called_number);
        ...
        println!("  Supported transports: {} types", stats.supported_transports.len());
        ...
        while event_count < max_events {
            match timeout(Duration::from_millis(100), self.event_receiver.recv()).await {
                Ok(Some(event)) => {
                    ...
                    match event {
                        DtmfEvent::DigitDetected { digit, duration, source, confidence, .. } => {
                            println!("🔢 Digit '{}' detected from {:?} (duration: {:?}, confidence: {:.2})", 
                                     digit, source, duration, confidence);
                        }
                        DtmfEvent::SequenceComplete { sequence, source, total_duration } => {
                            println!("✅ Sequence '{}' completed from {:?} (total: {:?})", 
                                     sequence, source, total_duration);
                        }
                        DtmfEvent::DigitGenerate { digit, duration, source, .. } => {
                            println!("🎵 Generate digit '{}' request from {:?} (duration: {:?})", 

...
impl RegulatoryRegistry {
    pub fn new() -> Self {
        ...
        regulatory_bodies.insert(
            ...
            RegulatoryBody {
                ...
                authority_name: "Canadian Radio-television and Telecommunications Commission"
                    .to_string(),
    ...
    pub fn should_enable_stir_shaken(
        &self,
        from_country: &str,
        to_country: &str,
    ) -> (bool, String) {
        ...
        if !has_from_regulatory && !has_to_regulatory {
            return (
                false,
                format!(
                    "No regulatory bodies found for {} or {}",
        ...
        if from_mandated || to_mandated {
            return (
                true,
                format!("STIR/SHAKEN mandated by {} or {}", from_country, to_country),
            );
        }
        ...
        if from_auth_required || to_auth_required {
            return (
                true,
                format!(
                    "Call authentication required by {} or {}",
                    from_country, to_country
                ),
            );
        }
...
fn main() {
    ...
    for (code, body) in &registry.regulatory_bodies {
        println!(
            "  {} ({}): {} - STIR/SHAKEN: {}, Auth Required: {}",
            code,
            body.authority_acronym,
            body.country_name,
            if body.stir_shaken_mandated {
                "✅ Mandated"
            } else {
                "❌ Not mandated"
            },
            if body.call_authentication_required {
                "✅ Required"
            } else {
                "❌ Not required"
            }
    ...
    for (from, to, description) in test_calls {
        ...
        let (should_enable, reason) =
            registry.should_enable_stir_shaken(&from_country, &to_country);

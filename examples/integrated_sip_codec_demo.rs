...
use redfire_switch::sip_codec_integration::{SipCodecIntegration, create_integrated_service};
use redfire_sip_stack::{SipCoreConfig, SipTransport};
use redfire_codec_engine::{CodecConfig, AudioCodec};
...
async fn main() -> Result<()> {
    ...
    println!("  • GPU acceleration: {}", 
             if cfg!(any(feature = "cuda", feature = "rocm")) { "Enabled" } else { "Disabled" });
    ...
    match service.process_sip_message(
        sample_invite.as_bytes(),
        from_addr,
        SipTransport::Udp,
    ).await {
        Ok(_) => {
...
async fn demonstrate_codec_translation() -> Result<()> {
    ...
    match service.transcode_frame(&session_id, sample_frame).await {
        Ok(transcoded) => {
            println!("  ✅ Transcoded {} bytes -> {} bytes", 
                    sample_frame.data.len(), transcoded.data.len());
        }
        Err(e) => {
...
fn demonstrate_sipi_integration() -> Result<()> {
    use redfire_sip_stack::{SipTSipIService, SipTSipIConfig};

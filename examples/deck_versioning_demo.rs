...
use redfire_switch::lcr::types::{DeckLoadRequest, RateType, NanpaRate, RouteRequest, RouteType};
    ...
    match deck_loader.get_decks_to_preload().await {
        Ok(schedules) => {
            ...
            for schedule in schedules {
                println!("  Schedule #{}: {} deck {} -> {} at {}", 
                    schedule.id, schedule.deck_type, 
                    schedule.current_deck_id, schedule.new_deck_id,
...
fn compare_rates(current: &NanpaRate, future: &NanpaRate) {
    ...
    println!("  Current: ${} -> Future: ${} ({})", 
        current.inter_rate, future.inter_rate,
        if future.inter_rate < current.inter_rate { "REDUCED" } else { "INCREASED" });
}
...
impl SipTools {
    ...
    pub async fn generate_sip_message(&self, args: Value) -> Result<Value> {
        ...
        let from_uri = args["from_uri"]
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("Missing from_uri"))?;

        let to_uri = args["to_uri"]
            .as_str()
        ...
        debug!(
            "Generating {} message from {} to {}",
            method, from_uri, to_uri
        );
        ...
        message.push_str(&format!(
            "Via: SIP/2.0/UDP localhost:5060;branch={}\r\n",
            branch
        ));
    ...
    fn parse_first_line(
        &self,
        first_line: &str,
    ) -> Result<(String, Option<String>, Option<u16>, Option<String>)> {
    ...
    fn check_required_headers(&self, headers: &HashMap<String, String>) -> bool {
        ...
        required.iter().all(|&header| {
            headers.contains_key(header)
                || headers
                    .keys()
                    .any(|k| k.to_lowercase() == header.to_lowercase())
        })
    }

    fn validate_uris(&self, headers: &HashMap<String, String>) -> bool {
        for (header, value) in headers {
            if header.to_lowercase() == "from"
                || header.to_lowercase() == "to"
                || header.to_lowercase() == "contact"
            {
    ...
    fn get_missing_required_headers(&self, headers: &HashMap<String, String>) -> Vec<String> {
        ...
        for &req_header in &required {
            if !headers.contains_key(req_header)
                && !headers
                    .keys()
                    .any(|k| k.to_lowercase() == req_header.to_lowercase())
            {

    ...
    pub fn to_string(&self) -> String {
        ...
        if let Some(method) = &self.method {
            ...
            result.push_str(&format!(
                ...
                self.version,
                self.status_code
                    .unwrap_or(SipStatusCode::ServerInternalError)
                    .as_u16(),
                self.reason_phrase
                    .as_ref()
...
impl SipParser {
    ...
    pub fn parse(&self, data: &[u8]) -> Result<SipMessage> {
        let text =
            std::str::from_utf8(data).map_err(|_| anyhow!("Invalid UTF-8 in SIP message"))?;
...
mod tests {
    ...
    fn test_sip_message_creation() {
        let mut msg =
            SipMessage::new_request(SipMethod::Invite, "sip:alice@example.com".to_string());
        ...
        assert_eq!(
            msg.get_header("From").unwrap().value,
